...
const ColorSelector: React.FC<ColorSelectorProps> = ({ onColorChange }) => {
    ...
    const currentIntensity = themeColorShade; const handleColorSelect = (color: TColors, intensity: TColorIntensity) => {
        if (onColorChange) {
                ...
                {intensities.map((intensity) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-6 h-6 rounded border transition-all ${currentIntensity === intensity
                                ? 'border-gray-800 dark:border-gray-200 scale-110'
                                : 'border-gray-300 hover:scale-105'
                                }`}
                            style={{ backgroundColor: colorValue }}
