...
export const useQuickProduct = ({
	...
	const [dedupProductsByBrand, setDedupProductsByBrand] = useState<
		Record<number, IDedupProductItem[]>
	>({});
	...
	const createQuickProduct = useCallback(
		async (data: IQuickProductForm, brandId: number): Promise<IProduct | null> => {
