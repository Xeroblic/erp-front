...
export const buildCategoryTableRows = (
  ...
  const visit = (category: ICategory, ancestorCategories: ICategory[]) => {
    ...
    const ancestorNames = normalizeParentNames(ancestorCategories.map((ancestor) => ancestor.name));
    const fallbackParentNames = normalizeParentNames(resolveFallbackParentNames(category, categoriesMap));

    const parentNames =
      parentNamesFromTree.length
        ? parentNamesFromTree
