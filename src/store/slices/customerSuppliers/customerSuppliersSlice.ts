...
const CUSTOMER_SUPPLIERS_PREFIX =
	(import.meta as any)?.env?.VITE_API_CUSTOMER_SUPPLIERS_PREFIX || '';
...
>(
	...
	async ({ subsidiaryId, customerSupplierId, search }, { rejectWithValue }) => {
		try {
			const resp = await ApiService.fetchData<{ data?: any[] }>({
				url: ep(
					`/subsidiaries/${subsidiaryId}/customer-suppliers/${customerSupplierId}/suppliers/`,
				),
				method: 'get',
...
export const attachSuppliersToCustomerSupplier = createAsyncThunk<
	ISupplier[],
	{
		...
		payload: IAttachSuppliersToCustomerSupplierRequest;
	},
	{ rejectValue: string }
>(
	...
	async ({ subsidiaryId, customerSupplierId, payload }, { rejectWithValue, dispatch }) => {
		try {
			...
			await ApiService.fetchData({
				url: ep(
					`/subsidiaries/${subsidiaryId}/customer-suppliers/${customerSupplierId}/attach-suppliers/`,
				),
				method: 'post',
				data: body,
			});
			const refreshed = await dispatch(
				fetchSuppliersOfCustomerSupplier({ subsidiaryId, customerSupplierId }),
			).unwrap();
			return refreshed;
		} catch (error: any) {
			return rejectWithValue(
...
export const detachSuppliersFromCustomerSupplier = createAsyncThunk<
	...
	{ rejectValue: string }
>(
	'customerSuppliers/detachSuppliers',
	async ({ subsidiaryId, customerSupplierId, payload }, { rejectWithValue, dispatch }) => {
		try {
			const body = { ...payload } as Record<string, unknown>;
			await ApiService.fetchData({
				url: ep(
					`/subsidiaries/${subsidiaryId}/customer-suppliers/${customerSupplierId}/detach-suppliers/`,
				),
				method: 'post',
				data: body,
			});
