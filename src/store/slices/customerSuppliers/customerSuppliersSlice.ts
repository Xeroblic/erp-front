...
export const fetchCustomerSuppliers = createAsyncThunk<ICustomerSupplier[], { subsidiaryId: number; search?: string; with_suppliers?: boolean }, { rejectValue: string }>(
...
export const fetchCustomerSupplierById = createAsyncThunk<ICustomerSupplier, { subsidiaryId: number; id: number }, { rejectValue: string }>(
...
export const updateCustomerSupplier = createAsyncThunk<ICustomerSupplier, { subsidiaryId: number; data: IUpdateCustomerSupplierRequest }, { rejectValue: string }>(
...
export const deleteCustomerSupplier = createAsyncThunk<number, { subsidiaryId: number; id: number }, { rejectValue: string }>(
	'customerSuppliers/delete',
	async ({ subsidiaryId, id }, { rejectWithValue }) => {
		try {
...
export const fetchSuppliersOfCustomerSupplier = createAsyncThunk<ISupplier[], { subsidiaryId: number; customerSupplierId: number; search?: string }, { rejectValue: string }>(
	'customerSuppliers/fetchSuppliers',
	async ({ subsidiaryId, customerSupplierId, search }, { rejectWithValue }) => {
		try {
			const resp = await ApiService.fetchData<{ data?: any[] }>({
...
export const attachSuppliersToCustomerSupplier = createAsyncThunk<ISupplier[], { subsidiaryId: number; customerSupplierId: number; payload: IAttachSuppliersToCustomerSupplierRequest }, { rejectValue: string }>(
	'customerSuppliers/attachSuppliers',
	async ({ subsidiaryId, customerSupplierId, payload }, { rejectWithValue, dispatch }) => {
		try {
			const body = { ...payload } as Record<string, unknown>;
			await ApiService.fetchData({
				url: ep(`/subsidiaries/${subsidiaryId}/customer-suppliers/${customerSupplierId}/attach-suppliers/`),
				method: 'post',
				data: body,
			});
...
export const detachSuppliersFromCustomerSupplier = createAsyncThunk<ISupplier[], { subsidiaryId: number; customerSupplierId: number; payload: IAttachSuppliersToCustomerSupplierRequest }, { rejectValue: string }>(
	'customerSuppliers/detachSuppliers', async ({ subsidiaryId, customerSupplierId, payload }, { rejectWithValue, dispatch }) => {
		try {
			const body = { ...payload } as Record<string, unknown>;
			await ApiService.fetchData({
				url: ep(`/subsidiaries/${subsidiaryId}/customer-suppliers/${customerSupplierId}/detach-suppliers/`),
				method: 'post',
				data: body,
			});
			const refreshed = await dispatch(fetchSuppliersOfCustomerSupplier({ subsidiaryId, customerSupplierId })).unwrap();
