...
export const listaRegionesThunk = createAsyncThunk<IRegion[], undefined, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await ApiService.fetchData<{ data?: any[] } | any>({ url: '/regions', method: 'get' });
...
export const listaProvinciasThunk = createAsyncThunk<IProvincia[], undefined, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await ApiService.fetchData<{ data?: any[] } | any>({ url: '/provinces', method: 'get' });
            const raw = Array.isArray(response.data?.data) ? response.data.data : Array.isArray(response.data) ? response.data : [];
...
export const listaComunasThunk = createAsyncThunk<IComuna[], undefined, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await ApiService.fetchData<{ data?: any[] } | any>({ url: '/communes', method: 'get' });
            const raw = Array.isArray(response.data?.data) ? response.data.data : Array.isArray(response.data) ? response.data : [];
