...
export const listaRegionesThunk = createAsyncThunk<IRegion[], undefined, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await ApiService.fetchData<IRegion[]>({ url: 'v1/regiones/', method: 'get' })
...
export const listaProvinciasThunk = createAsyncThunk<IProvincia[], undefined, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await ApiService.fetchData<IProvincia[]>({ url: 'v1/provincias/', method: 'get' })
            return response.data
        } catch (error: any) {
            return rejectWithValue(error)
        }
...
export const listaComunasThunk = createAsyncThunk<IComuna[], undefined, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await ApiService.fetchData<IComuna[]>({ url: 'v1/comunas/', method: 'get' })
            return response.data
        } catch (error: any) {
            return rejectWithValue(error)
        }
