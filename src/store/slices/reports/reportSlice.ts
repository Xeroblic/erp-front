...
export const reportsSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(fetchReportResults.fulfilled, (state, action) => {
			...
			console.log(
				'[reportSlice] fetchReportResults.fulfilled - action.payload:',
				action.payload,
			);
			console.log(
				'[reportSlice] fetchReportResults.fulfilled - action.payload type:',
				typeof action.payload,
			);
			console.log(
				'[reportSlice] fetchReportResults.fulfilled - action.payload es array:',
			...
			if (
				...
				console.log(
					'[reportSlice] fetchReportResults.fulfilled - action.payload keys:',
					Object.keys(action.payload),
				);
				console.log(
					'[reportSlice] fetchReportResults.fulfilled - action.payload.data:',
					(action.payload as any)?.data,
				);
				console.log(
					'[reportSlice] fetchReportResults.fulfilled - action.payload.data length:',
					Array.isArray((action.payload as any)?.data)
						? (action.payload as any).data.length
						: 'N/A',
				);
			}
			state.results = action.payload as IReportResult<any>;
			console.log(
				'[reportSlice] fetchReportResults.fulfilled - state.results después de asignar:',
				state.results,
			);
			state.loading = false;
			state.error = null;
