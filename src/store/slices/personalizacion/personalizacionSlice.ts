...
>(
  ...
  async (_, { getState, rejectWithValue, signal }) => {
    try {
      ...
      const personalizationData =
        resp.data.personalization ?? resp.data;
  ...
  },
  {
    condition: (_, { getState }) => {
      ...
      const hasToken =
        !!tokenManager.getAccessToken() || !!state.auth.access;
...
>(
  ...
  async (data, { getState, rejectWithValue }) => {
    const token =
      tokenManager.getAccessToken() ?? getState().auth.access;
...
const personalizacionSlice = createSlice({
  ...
  reducers: {
    ...
    syncWithApiData: (
      state,
      action: PayloadAction<IPersonalizacionUsuario>,
    ) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(actualizarPersonalizacionThunk.rejected, (state, action) => {
        ...
        state.error = action.payload as string | undefined;
        toast.error(
          (action.payload as string) || 'Error al actualizar personalización',
        );
      })
...
/* --------- Selectores --------- */
const getSlice = (s: RootState): PersonalizacionState =>
  s.personalizacion ?? getInitialState();
...
export const selectThemeColorShade = (s: RootState) =>
  getSlice(s).themeColorShade;
...
export const selectHasUnsavedChanges = (s: RootState) =>
  getSlice(s).hasUnsavedChanges;

export const selectIsInitialized = (s: RootState) =>
  getSlice(s).isInitialized;

export const selectIsDarkTheme = (s: RootState) => {
  const darkMode = getSlice(s).darkMode;
  const isSystemDark =
