        ...
        <Modal isOpen={isOpen} setIsOpen={handleSetOpen} size='sm' isCentered>
            <ModalHeader>
                <div className='flex flex-col'>
                    <span>{title}</span>
                    {module && <span className='text-sm font-normal text-zinc-500 dark:text-zinc-400'>{module}</span>}
                </div>
            </ModalHeader>
            <ModalBody>
                <div className='text-zinc-800 dark:text-zinc-200 whitespace-pre-line'>
                    {message}
                </div>
            </ModalBody>
            <ModalFooter>
                <ModalFooterChild>
                    <Button variant='default' onClick={handleMarkRead} icon='HeroEnvelopeOpen'>Marcar leída</Button>
                    <Button variant='outline' onClick={handleAck} icon='HeroArchiveBox'>Archivar</Button>
                </ModalFooterChild>
                <ModalFooterChild>
                    <Button variant='solid' onClick={onClose} icon='HeroCheckCircle'>Entendido</Button>
                </ModalFooterChild>
            </ModalFooter>
        </Modal>
    );
