...
export const normalizeInitialGeoFromUser = (
	user?: IUserMe | (IUserMe & Record<string, any>) | null,
): GeoInit => {
...
export const getAvatarUrlFromUser = (
	user?: IUserMe | (IUserMe & Record<string, any>) | null,
): string | null => {
	...
	return candidates.find((item: unknown) => typeof item === 'string' && item.length > 0) ?? null;
};
...
export const buildProfileUpdatePayload = (
	...
	toApiDate: (v?: string) => string | null,
	toGenderApiValueFn: (v?: string | null) => string | null,
): ProfilePayload => {
