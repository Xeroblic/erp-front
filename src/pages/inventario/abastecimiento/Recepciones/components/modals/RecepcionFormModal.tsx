...
import { procurementWarehouses, purchasableProcurementProducts } from '@/mocks/db/procurement.db';
...
const RecepcionFormModal: React.FC<IRecepcionFormModalProps> = ({
	...
	const { formik, isEdit, isSubmitting, hasVersionConflict, reset } = useRecepcionForm({
		subsidiaryId,
		branchId,
		defaultWarehouseId,
		receipt,
				...
				<form
					...
					<ModalBody className='space-y-4'>
						...
						<div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
							...
							<div className='space-y-1'>
								<Label htmlFor='recepcion-received-on'>Fecha de recepción</Label>
								<Input
								...
								{existingReceiptIsDraftOnly(isEdit, receipt) && (
									<p className='text-xs text-zinc-500'>
										Sólo se cambia mientras está en borrador.
									</p>
								)}
							</div>
							...
							{!isManual && (
								<div className='space-y-1 sm:col-span-2'>
									<Label htmlFor='recepcion-document'>Documento confirmado</Label>
									<SelectReact
									...
									/>
									<p className='text-xs text-zinc-500'>
										Producto, proveedor y costo se derivan del documento: no se
										pueden sobrescribir.
									</p>
								</div>
							)}
							...
							{isManual && (
								<div className='space-y-1'>
									<Label htmlFor='recepcion-supplier'>Proveedor (opcional)</Label>
									<SelectReact
										name='supplier_id'
									...
									/>
									<p className='text-xs text-zinc-500'>
										Con proveedor conocido, el costo unitario es obligatorio.
									</p>
								</div>
							)}
					...
					<ModalFooter>
						...
						<Button
							...
							{isEdit ? 'Guardar corrección' : 'Crear recepción'}
						</Button>
					</ModalFooter>
				</form>
			</FormikProvider>
