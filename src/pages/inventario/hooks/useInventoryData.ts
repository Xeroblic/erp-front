...
export const useInventoryData = () => {
    ...
    const getMovements = (filters?: {
        ...
        if (filters) {
            ...
            if (filters.productId) {
                filteredMovements = filteredMovements.filter(m => m.product.id === filters.productId);
            }
            if (filters.warehouseId) {
                filteredMovements = filteredMovements.filter(m => m.warehouse.id === filters.warehouseId);
            }
            if (filters.dateFrom) {
                filteredMovements = filteredMovements.filter(m =>
                    new Date(m.movement_date) >= new Date(filters.dateFrom!)
                );
            }
            if (filters.dateTo) {
                filteredMovements = filteredMovements.filter(m =>
                    new Date(m.movement_date) <= new Date(filters.dateTo!)
                );
            }
        }
    ...
    const getItems = (filters?: {
        ...
        if (filters) {
            ...
            if (filters.warehouseId) {
                filteredItems = filteredItems.filter(item => item.warehouse.id === filters.warehouseId);
            }
            if (filters.productId) {
                filteredItems = filteredItems.filter(item => item.product.id === filters.productId);
            }
            if (filters.lowStock) {
    ...
    const getStockLevels = (filters?: {
        ...
        if (filters) {
            ...
            if (filters.warehouseId) {
                filteredLevels = filteredLevels.filter(level => level.warehouse.id === filters.warehouseId);
            }
            if (filters.productId) {
                filteredLevels = filteredLevels.filter(level => level.product.id === filters.productId);
            }
        }
