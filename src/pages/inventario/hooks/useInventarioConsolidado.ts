...
const MOCK_ESTADISTICAS = {
	...
	movimientosEntrada: MOCK_MOVIMIENTOS.filter(
		(mov) => mov.movement_type === 'IN' || mov.quantity > 0,
	).length,
	movimientosSalida: MOCK_MOVIMIENTOS.filter(
		(mov) => mov.movement_type === 'OUT' || mov.quantity < 0,
	).length,
...
export const useInventoryData = () => {
	...
	const getMovements = async (filtros: any = {}) => {
		...
		if (filtros.bodega) {
			movimientos = movimientos.filter(
				(mov) => mov.warehouse?.id.toString() === filtros.bodega,
			);
		}
		...
		if (filtros.fechaDesde) {
			movimientos = movimientos.filter(
				(mov) => new Date(mov.movement_date) >= new Date(filtros.fechaDesde),
			);
		}

		if (filtros.fechaHasta) {
			movimientos = movimientos.filter(
				(mov) => new Date(mov.movement_date) <= new Date(filtros.fechaHasta),
			);
		}
	...
	const transferStock = async (transferData: {
		...
		if (product && fromWarehouse && toWarehouse) {
			...
			const fromItem = MOCK_STOCK_ITEMS.find(
				(item) =>
					item.product_id === transferData.productId &&
	...
	const editMovement = async (
		...
		if (
			originalMovement.reference?.includes('SALE-') &&
			originalMovement.movement_type === 'OUT'
		) {
		...
		const originalStockItem = MOCK_STOCK_ITEMS.find(
			(item) =>
				item.product_id === originalMovement.product.id &&
		...
		if (
			(updatedData.movement_type === 'OUT' || updatedData.movement_type === 'TRANSFER') &&
			finalQuantity < 0 &&
		...
		if (originalStockItem && originalStockItem.id !== newStockItem.id) {
			...
			originalStockItem.total_value =
				originalStockItem.current_stock * originalStockItem.unit_cost;
		...
		const updatedMovement = {
			...
			total_cost: updatedData.unit_cost
				? Math.abs(finalQuantity) * updatedData.unit_cost
	...
	const deleteMovement = async (movementId: number) => {
		...
		if (
			movementToDelete.reference?.includes('SALE-') &&
			movementToDelete.movement_type === 'OUT'
		) {
			throw new Error(
				'No se puede eliminar: el movimiento está asociado a una venta cerrada',
			);
		}
		...
		const hasReferences = MOCK_MOVIMIENTOS.some(
			(mov) =>
				mov.id !== movementId &&
		...
		if (hasReferences && movementToDelete.reference?.includes('TRF-')) {
			throw new Error(
				'No se puede eliminar: el movimiento es parte de una transferencia con movimientos relacionados',
			);
		}
		...
		const stockItem = MOCK_STOCK_ITEMS.find(
			(item) =>
				item.product_id === movementToDelete.product.id &&
		...
		if (stockItem) {
			...
			if (movementToDelete.movement_type === 'IN' || movementToDelete.quantity > 0) {
				...
				if (stockItem.current_stock < quantityToRevert) {
					throw new Error(
						'No se puede eliminar: la reversión del movimiento resultaría en stock negativo. ' +
	...
	const getWarranties = async (filtros: any = {}) => {
		...
		if (filtros.producto) {
			warranties = warranties.filter(
				(warranty) => warranty.product.id === parseInt(filtros.producto),
			);
		}

		if (filtros.tipoGarantia) {
			warranties = warranties.filter(
				(warranty) => warranty.warranty_type === filtros.tipoGarantia,
			);
		}
		...
		if (filtros.fechaDesde) {
			warranties = warranties.filter(
				(warranty) => new Date(warranty.start_date) >= new Date(filtros.fechaDesde),
			);
		}

		if (filtros.fechaHasta) {
			warranties = warranties.filter(
				(warranty) => new Date(warranty.end_date) <= new Date(filtros.fechaHasta),
			);
		}
	...
	const createWarranty = async (warrantyData: {
		...
		if (
			...
			throw new Error(
				'Campos obligatorios: Producto, Tipo de garantía, Fecha de inicio, Fecha de término, Estado',
			);
		}
		...
		if (warrantyData.claim_date) {
			...
			if (claimDate < startDate || claimDate > endDate) {
				throw new Error(
					'La fecha de reclamo debe estar entre la fecha de inicio y término',
				);
			}
		}
		...
		if (warrantyData.resolution_date && warrantyData.claim_date) {
			...
			if (resolutionDate < claimDate) {
				throw new Error(
					'La fecha de resolución debe ser mayor o igual a la fecha de reclamo',
				);
			}
		}
		...
		// Validar duplicidad
		const duplicate = MOCK_GARANTIAS.find(
			(warranty) =>
		...
		if (duplicate) {
			throw new Error(
				'Ya existe una garantía con la misma combinación de producto, tipo y fechas',
			);
		}
	...
	const editWarranty = async (
		...
		if (duplicate) {
			throw new Error(
				'Ya existe otra garantía con la misma combinación de producto, tipo y fechas',
			);
		}
	...
	const deleteWarranty = async (warrantyId: number) => {
		...
		if (warrantyToDelete.status === 'Usada' && warrantyToDelete.resolution_date) {
			throw new Error(
				'No se puede eliminar: la garantía fue usada y tiene resolución registrada (auditoría)',
			);
		}
