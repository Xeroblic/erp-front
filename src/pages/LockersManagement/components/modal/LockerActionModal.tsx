...
interface ILockerActionModalProps {
    ...
    onClose: () => void;
    onSubmit: (data: { orderId: number | null; pinManual: string; actionLocker: ILockerInternal | null }) => void;
}
...
const LockerActionModal: React.FC<ILockerActionModalProps> = ({
    ...
    const lockerOptions: TSelectOption[] = useMemo(() => {
        return lockers
            .filter((l) => l.status === 'available' || l.status === 'maintenance' || l.id === actionLocker?.id)
            .map((l) => ({
                value: String(l.id),
            ...
            <ModalBody>
                <div className='space-y-4'>
                                ...
                                {actionType === 'dropoff' ? (
                                    <div className='col-span-2'>
                                        ...
                                        <SelectReact
                                            ...
                                            value={
                                                lockerOptions.find((o) => o.value === String(actionLocker.id)) || null
                                            }
                                            onChange={(opt) => {
                    ...
                    <div className='rounded-lg border border-zinc-200 bg-white p-3 text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-400'>
                        {actionType === 'withdraw' && (
                            <p>
                                <strong>Fase 2:</strong> Retira el equipo del casillero. Se generará un nuevo PIN automáticamente y el casillero quedará{' '}
                                <strong>Disponible</strong>.
                            </p>
                        )}
                        {actionType === 'dropoff' && (
                            <p>
                                <strong>Fase 4:</strong> Deposita el equipo reparado en este casillero. Se generará un nuevo PIN y el estado cambiará a{' '}
                                <strong>Esperando Retiro</strong>. El cliente recibirá una notificación.
                            </p>
                        )}
                        {actionType === 'reset' && (
                            <p>
                                <strong>Fase 6:</strong> Resetea el casillero después del retiro del cliente. Se genera un nuevo PIN y el casillero vuelve a estar{' '}
                                <strong>Disponible</strong>.
                            </p>
                        )}
                        {actionType === 'ready' && (
                            <p>Marca la orden como lista para que el cliente retire. Ingresa el PIN que se le comunicará al cliente.</p>
                        )}
                    </div>
                        ...
                        <div>
                            ...
                            {initialOrderId ? (
                                <div className='rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300'>
                                    {orderOptions.find((o) => o.value === String(initialOrderId))?.label || `Orden #${initialOrderId}`}
                                </div>
                                ...
                                <SelectReact
                                    ...
                                    value={orderOptions.find((o) => o.value === String(selectedOrderId)) ?? null}
                                    onChange={(opt) => {
                                        const selected = opt as TSelectOption | null;
                                        setSelectedOrderId(selected ? Number(selected.value) : null);
                                    }}
                                />
                            )}
                        </div>
                    ...
                    {actionType === 'ready' && (
                        <div>
                            ...
                            />
                            <p className='mt-1 text-xs text-zinc-400'>Este PIN se le enviará al cliente para que retire su equipo.</p>
                        </div>
                    )}
                </div>
            </ModalBody>
