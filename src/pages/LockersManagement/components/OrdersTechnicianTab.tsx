...
const OrdersTechnicianTab: React.FC<IOrdersTechnicianTabProps> = ({
	...
	const ordersToDropoff = serviceOrders.filter((order) => {
		...
		if (order.locker_id) {
			...
			if (
				assignedLocker &&
				assignedLocker.status !== 'available' &&
				assignedLocker.status !== 'Disponible'
			) {
		...
		<div className='grid grid-cols-1 gap-8 lg:grid-cols-2'>
				...
				<div className='flex items-center gap-3 border-b border-zinc-200 pb-3 dark:border-zinc-700'>
					...
					<div>
						<h2 className='text-lg font-bold text-zinc-800 dark:text-zinc-100'>
							Equipos para Retirar
						</h2>
						<p className='text-xs text-zinc-500'>
							Equipos que el cliente dejó en el casillero
						</p>
					</div>
				</div>

				{lockersToWithdraw.length === 0 ? (
					<div className='rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 py-12 text-center dark:border-zinc-700 dark:bg-zinc-800/50'>
						<Icon
							icon='HeroCheckCircle'
							className='mx-auto mb-3 h-12 w-12 text-zinc-300'
						/>
					...
					lockersToWithdraw.map((locker) => {
						const order =
							locker.active_service_order ||
							serviceOrders.find((o) => o.locker_id === locker.id);
						return (
							<Card
								key={locker.id}
								...
								<CardBody className='p-5'>
									<div className='mb-4 flex items-start justify-between'>
										<div>
											...
											<h3 className='text-lg font-bold uppercase text-zinc-800 dark:text-zinc-100'>
												{order?.customer_name ||
									...
									<div className='mb-5 rounded-lg border border-zinc-100 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800'>
										<p className='flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300'>
											<Icon
												icon='HeroDevicePhoneMobile'
												className='mr-2 inline h-4 w-4 text-zinc-400'
											/>
			...
			<div className='space-y-4'>
				<div className='flex items-center gap-3 border-b border-zinc-200 pb-3 dark:border-zinc-700'>
					...
					<div>
						<h2 className='text-lg font-bold text-zinc-800 dark:text-zinc-100'>
							Equipos para Depositar
						</h2>
						<p className='text-xs text-zinc-500'>
							Equipos listos para que el cliente retire
						</p>
					</div>
				</div>
				...
				{ordersToDropoff.length === 0 ? (
					<div className='rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 py-12 text-center dark:border-zinc-700 dark:bg-zinc-800/50'>
						...
						/>
						<p className='font-medium text-zinc-500'>
							No hay órdenes listas para depositar.
						</p>
					</div>
				) : (
					ordersToDropoff.map((order) => (
						<Card
							key={order.id}
							className='border-l-4 border-l-emerald-500 transition-shadow hover:shadow-md'>
							<CardBody className='p-5'>
								<div className='mb-4 flex items-start justify-between'>
									<div>
										<Badge
