...
import { buildInitialValues, buildSubmitPayload, createBrandOptions, createCategoryOptions } from '../../utils/productForm.utils';
        ...
        <Modal isOpen={isOpen} setIsOpen={onClose} size='md'>
            <ModalHeader>
                <div className='flex items-center gap-3'>
                    <Icon icon={product ? 'HeroPencilSquare' : 'HeroSparkles'} className='h-5 w-5' />
                    <div>
                        <p className='text-lg font-semibold'>{product ? 'Editar producto' : 'Nuevo producto'}</p>
                        <p className='text-sm text-neutral-500'>Completa la informacion para sincronizar el producto.</p>
                    </div>
                </div>
            </ModalHeader>
            <Formik initialValues={initialValues} validationSchema={productFormSchema} enableReinitialize onSubmit={handleSubmit}>
                    ...
                    <Form>
                                            ...
                                            <>
												<Field name='sku'>
													{({ field, meta }: FieldProps) => (
														<FieldContainer id='sku' label='SKU' error={meta.touched ? (meta.error as string) : undefined}>
															<Input id='sku' {...field} required disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
												<Field name='name'>
													{({ field, meta }: FieldProps) => (
														<FieldContainer id='name' label='Nombre' error={meta.touched ? (meta.error as string) : undefined}>
															<Input id='name' {...field} required disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
                                                <div className='md:col-span-2 space-y-2'>
                                                    ...
                                                    <SelectReact
                                                        ...
                                                        value={brandOptions.find((option) => option.value === values.brand_id) ?? null}
                                                        onChange={(option) =>
                                                            setFieldValue('brand_id', (option as TSelectOption | null)?.value ?? '')
                                                        }
                                                        onBlur={() => setFieldTouched('brand_id', true)}
                                                        placeholder='Selecciona una marca'
                                                        isDisabled={isBusy || !brandOptions.length}
												...
												<Field name='uom'>
													{({ field }: FieldProps) => (
														<FieldContainer id='uom' label='Unidad de medida'>
															<Input id='uom' {...field} disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
												<Field name='condition_policy'>
													{({ field }: FieldProps) => (
														<FieldContainer id='condition_policy' label='Condicion'>
															<Input id='condition_policy' {...field} disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
                                            </>
												...
												<Field name='price'>
													{({ field, meta }: FieldProps) => (
														<FieldContainer
															...
															<Input id='price' type='number' min='0' {...field} required disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
												<Field name='cost'>
													{({ field }: FieldProps) => (
														<FieldContainer id='cost' label='Costo'>
															<Input id='cost' type='number' min='0' {...field} disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
												<Field name='offer_price'>
													{({ field }: FieldProps) => (
														<FieldContainer id='offer_price' label='Precio oferta'>
															<Input id='offer_price' type='number' min='0' {...field} disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
												<Field name='warranty_months'>
													{({ field }: FieldProps) => (
														<FieldContainer id='warranty_months' label='Meses de garantia'>
															<Input id='warranty_months' type='number' min='0' {...field} disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
                                            </>
												...
												<Field name='product_type'>
													{({ field, meta }: FieldProps) => (
														<FieldContainer
															...
															error={meta.touched ? (meta.error as string) : undefined}>
															<Input id='product_type' {...field} disabled={isBusy} />
														</FieldContainer>
													)}
												</Field>
                                                <div className='md:col-span-2 space-y-2'>
                                                    ...
                                                    <SelectReact
                                                        ...
                                                        onChange={(option) =>
                                                            setFieldValue(
                                                                'categories',
                                                                ((option as TSelectOption[]) ?? []).map((item) => ({
                                                                    value: item.value,
                                                                    label: item.label,
                                                                })),
                                                            )
                                                        ...
                                                        isDisabled={isBusy || !categoryOptions.length}
                            ...
                            <Card>
                                <CardBody className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                                    {PRODUCT_TOGGLES.map((toggle) => (
                                        <div key={toggle.key} className='flex items-center gap-4 rounded-lg border p-4'>
                                            ...
                                            <Checkbox
                                                checked={values[toggle.key as ProductToggleKey] as boolean}
                                                onChange={(event) => setFieldValue(toggle.key, event.target.checked)}
                                                disabled={isBusy}
                                            />
                                        </div>
                                    ))}
                        ...
                        <ModalFooter>
                            <div className='flex w-full justify-end gap-3'>
                                <Button variant='outline' onClick={onClose} isDisable={isBusy} icon='HeroXMark'>
                                    Cancelar
                                </Button>
                                <Button
                                    type='submit'
