...
import type { IProduct, ProductInventorySummary, ProductListMeta } from '@/interface/product.interface';
  ...
  return (
    <div className='relative flex h-48 w-48 items-center justify-center self-center rounded-full bg-white dark:bg-neutral-950'>
      ...
      <div className='relative z-10 text-center'>
        <p className='text-xs font-medium uppercase tracking-[0.2em] text-neutral-400'>Seriales</p>
...
const InventoryTab: React.FC<InventoryTabProps> = ({
  ...
  const currentPageInventory = useMemo(() => {
    ...
    return {
      ...
      childCount,
      visibleStock,
      from: activeMeta.total === 0 ? 0 : (activeMeta.current_page - 1) * activeMeta.per_page + 1,
  ...
  const trackedRatio = summaryData.productsTotalAll > 0
    ? (summaryData.serialTrackingCount / summaryData.productsTotalAll) * 100
    : 0;
  const untrackedRatio = summaryData.productsTotalAll > 0
    ...
    <div className='space-y-6'>
                        ...
                        <div
                          ...
                          <div className='flex items-start justify-between gap-3'>
                            ...
                            <div className='flex h-11 w-11 items-center justify-center rounded-xl bg-white shadow-sm dark:bg-neutral-900'>
                              <Icon icon={card.icon} className={`h-5 w-5 ${card.tone}`} />
                            </div>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className='grid gap-4'>
                      <div className='rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900/60'>
                        <div className='flex items-center justify-between gap-3'>
                          <div>
                            <p className='text-xs font-semibold uppercase tracking-wide text-neutral-400'>Brand portfolio</p>
                            <p className='mt-1 text-sm text-neutral-500'>Peso de la marca visible en esta página.</p>
                          </div>
                          <Icon icon='HeroBuildingOffice2' className='h-5 w-5 text-neutral-400' />
                        </div>
                        <div className='mt-4 space-y-3'>
                          {Object.entries(
                            visibleInventoryRows.reduce<Record<string, number>>((acc, row) => {
                              acc[row.brand] = (acc[row.brand] ?? 0) + row.stock;
                              return acc;
                            }, {}),
                          )
                            .sort((a, b) => b[1] - a[1])
                            .slice(0, 4)
                            .map(([brand, stock]) => {
                              const pct = summaryData.stockTotal > 0 ? (stock / summaryData.stockTotal) * 100 : 0;
                              return (
                                <div key={brand} className='space-y-1.5'>
                                  <div className='flex items-center justify-between gap-3 text-sm'>
                                    <span className='font-medium text-neutral-700 dark:text-neutral-200'>{brand}</span>
                                    <span className='text-neutral-500'>{formatNumber(stock)} units</span>
                                  </div>
                      ...
                      <div className='rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/30 dark:bg-amber-500/10'>
                        <div className='flex items-start gap-3'>
                          ...
                          <div>
                            <p className='text-sm font-semibold text-amber-700 dark:text-amber-200'>Restock analysis</p>
              ...
              <Tab
                ...
                <div className='grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]'>
                  <div className='rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900/60'>
                    <div className='flex items-center justify-between gap-3'>
                      <div>
                        <p className='text-xs font-semibold uppercase tracking-wide text-neutral-400'>Pie chart</p>
                        <p className='mt-1 text-sm text-neutral-500'>Estado actual de seriales aprobados.</p>
                      </div>
                      <Badge variant='outline' color='violet'>{formatNumber(serialsTotal)}</Badge>
                    </div>
                    <div className='mt-4 flex justify-center'>
                      <CircularMetricChart segments={serialSegments} total={serialsTotal} />
                    </div>
                  </div>
                  <div className='space-y-4'>
                    <div className='rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900/60'>
                      <div className='flex items-center justify-between gap-3'>
                        <div>
                          <p className='text-xs font-semibold uppercase tracking-wide text-neutral-400'>Dashboard serializado</p>
                          <p className='mt-1 text-sm text-neutral-500'>Productos con tracking, disponibilidad y salida comercial.</p>
                        </div>
                        <Icon icon='HeroChartPie' className='h-5 w-5 text-violet-400' />
                      </div>
                      <div className='mt-4'><SerialsDistributionBar segments={serialSegments} total={serialsTotal} /></div>
                    ...
                    <div className='grid gap-4 md:grid-cols-3'>
                      <div className='rounded-2xl border border-neutral-200 bg-emerald-50 p-4 dark:border-emerald-500/30 dark:bg-emerald-500/10'>
                        <p className='text-xs font-semibold uppercase tracking-wide text-emerald-600 dark:text-emerald-300'>Disponibles</p>
                        <p className='mt-2 text-3xl font-semibold text-emerald-700 dark:text-emerald-200'>{formatNumber(summaryData.serialsAvailable)}</p>
                        <p className='mt-1 text-xs text-emerald-700/80 dark:text-emerald-100/80'>Seriales listos para operar</p>
                      </div>
                      <div className='rounded-2xl border border-neutral-200 bg-violet-50 p-4 dark:border-violet-500/30 dark:bg-violet-500/10'>
                        <p className='text-xs font-semibold uppercase tracking-wide text-violet-600 dark:text-violet-300'>Vendidos</p>
                        <p className='mt-2 text-3xl font-semibold text-violet-700 dark:text-violet-200'>{formatNumber(summaryData.serialsSold)}</p>
                        <p className='mt-1 text-xs text-violet-700/80 dark:text-violet-100/80'>{serialSoldRatio.toFixed(1)}% del total aprobado</p>
                      </div>
                      <div className='rounded-2xl border border-neutral-200 bg-sky-50 p-4 dark:border-sky-500/30 dark:bg-sky-500/10'>
                        <p className='text-xs font-semibold uppercase tracking-wide text-sky-600 dark:text-sky-300'>Tracking coverage</p>
                        <p className='mt-2 text-3xl font-semibold text-sky-700 dark:text-sky-200'>{formatNumber(summaryData.serialTrackingCount)}</p>
                        <p className='mt-1 text-xs text-sky-700/80 dark:text-sky-100/80'>Productos controlados por serial</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <Tab
                ...
                <div className='grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]'>
                  <div className='rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900/60'>
                    <div className='flex items-start justify-between gap-3'>
                      <div>
                        <p className='text-xs font-semibold uppercase tracking-wide text-neutral-400'>Stock no serializado</p>
                        <p className='mt-1 text-sm text-neutral-500'>Unidades disponibles sin serial asignado, separadas de las métricas serializadas.</p>
                      </div>
                      <Icon icon='HeroArchiveBoxXMark' className='h-5 w-5 text-amber-400' />
                    </div>
                    <div className='mt-5 grid gap-4 sm:grid-cols-3'>
                      <div className='rounded-xl border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-950/50'>
                        <p className='text-xs uppercase tracking-wide text-neutral-400'>Stock sin serie</p>
                        <p className='mt-2 text-3xl font-semibold text-neutral-900 dark:text-neutral-100'>{formatNumber(summaryData.stockWithoutSerials)}</p>
                      </div>
                      <div className='rounded-xl border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-950/50'>
                        <p className='text-xs uppercase tracking-wide text-neutral-400'>Productos sin tracking</p>
                        <p className='mt-2 text-3xl font-semibold text-neutral-900 dark:text-neutral-100'>{formatNumber(summaryData.withoutSerialTracking)}</p>
                      </div>
                      <div className='rounded-xl border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-950/50'>
                        <p className='text-xs uppercase tracking-wide text-neutral-400'>Participación</p>
                        <p className='mt-2 text-3xl font-semibold text-neutral-900 dark:text-neutral-100'>{summaryData.stockTotal > 0 ? ((summaryData.stockWithoutSerials / summaryData.stockTotal) * 100).toFixed(1) : '0.0'}%</p>
                      </div>
                    </div>
                  </div>
                  <div className='rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/30 dark:bg-amber-500/10'>
                    <p className='text-xs font-semibold uppercase tracking-wide text-amber-600 dark:text-amber-300'>Observación</p>
                    <p className='mt-2 text-sm text-amber-700 dark:text-amber-100'>
                      Este tab te deja separado el inventario no serializado para que no contamine el análisis de seriales vendidos, reservados o aprobados.
                    </p>
                  </div>
                </div>
              </Tab>
            ...
            <div className='flex flex-wrap gap-2'>
              {canUseInventoryImportExport ? (
                ...
                <div className='rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300'>
                  Los productos con serial tracking se gestionan desde el módulo de equipos. En esta vista no se habilita importar ni exportar inventario.
                </div>
              )}
      ...
      <Card className='border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900/70'>
        <CardHeader>
          ...
          <CardHeaderChild>
            <Badge variant='outline' color='sky'>Página {formatNumber(activeMeta.current_page)} de {formatNumber(activeMeta.last_page)}</Badge>
          </CardHeaderChild>
        </CardHeader>
        <CardBody>
          ...
          <div className='mb-4 rounded-xl border border-dashed border-neutral-200 bg-neutral-50 p-4 text-sm dark:border-neutral-700 dark:bg-neutral-900/30'>
            <p className='font-medium text-neutral-700 dark:text-neutral-200'>{visibleRowsDescription}</p>
            <p className='mt-1 text-xs text-neutral-500'>La API pagina {formatNumber(activeMeta.total)} productos padre. En esta página se están viendo del {formatNumber(currentPageInventory.from)} al {formatNumber(currentPageInventory.to)}, que despliegan {formatNumber(currentPageInventory.childCount)} variantes adicionales. Vista actual: {formatNumber(visibleInventoryRows.length)} registros filtrados por tab.</p>
          </div>
          ...
          {activeTableLoading ? (
            ...
            <div className='rounded-lg border border-dashed border-neutral-200 p-6 text-center text-sm text-neutral-500 dark:border-neutral-700'>No hay productos visibles para los filtros aplicados en esta pestaña.</div>
          ) : (
