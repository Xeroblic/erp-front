...
const buildPreviewFromCsv = (content: string): CsvPreview => {
	...
	const parsedRows = rows.slice(1).map((row, index) => {
		...
		const quantityChange =
			quantityChangeIndex >= 0 ? String(row[quantityChangeIndex] ?? '').trim() : '';
		const serialTrackingRaw =
			serialTrackingIndex >= 0
				? String(row[serialTrackingIndex] ?? '')
						.trim()
						.toLowerCase()
				: '';
	...
	return {
		...
		serialTrackingHeader:
			serialTrackingIndex >= 0 ? (headers[serialTrackingIndex] ?? null) : null,
		delimiter,
	};
};
...
const getPreviewRowState = (
	...
	if (!Number.isFinite(productIdNumber) || productIdNumber <= 0)
		return { kind: 'invalid', message: 'product_id inválido' };
	...
	if (row.sourceSerialTracking === true) {
		return {
			kind: 'blocked',
			message: 'Producto serializado según CSV: usar módulo de equipos',
		};
	}
	if (serialTrackedProductIds.has(productIdNumber)) {
		return { kind: 'blocked', message: 'Producto serializado: usar módulo de equipos' };
	}
	if (row.quantityChange.trim().length === 0)
		return { kind: 'omitted', message: 'Sin ajuste: no se importará' };
	if (!Number.isFinite(quantityChangeNumber))
		return { kind: 'invalid', message: 'quantity_change inválido' };
	if (quantityChangeNumber === 0)
		return { kind: 'omitted', message: 'Ajuste en 0: no se importará' };
...
const InventoryImportModal: React.FC<InventoryImportModalProps> = ({
	...
	const serialTrackedIdsSet = useMemo(
		() => new Set(serialTrackedProductIds),
		[serialTrackedProductIds],
	);
	...
	const validPreviewRows = useMemo(
		() =>
			preview?.rows.filter(
				(row) => getPreviewRowState(row, serialTrackedIdsSet).kind === 'importable',
			) ?? [],
	...
	const blockedPreviewRows = useMemo(
		() =>
			preview?.rows.filter(
				(row) => getPreviewRowState(row, serialTrackedIdsSet).kind === 'blocked',
			) ?? [],
		[preview, serialTrackedIdsSet],
	);
	...
	const omittedPreviewRows = useMemo(
		() =>
			preview?.rows.filter(
				(row) => getPreviewRowState(row, serialTrackedIdsSet).kind === 'omitted',
			) ?? [],
		[preview, serialTrackedIdsSet],
	);
	...
	const handleFileSelection = async (file: File | null) => {
		...
		if (!nextPreview.productIdHeader || !nextPreview.quantityHeader) {
			toast.warning(
				'No se detectaron automáticamente todas las columnas. Revisa y corrige la preview antes de importar.',
			);
			return;
		}
		const nextRowStates = nextPreview.rows.map((row) =>
			getPreviewRowState(row, serialTrackedIdsSet),
		);
		...
		if (blockedCount > 0 && blockedCount === nextPreview.rows.length) {
			toast.warning(
				'Este CSV contiene solo productos serializados. Debes moverlos desde el módulo de equipos.',
			);
			return;
		}
	...
	const handleImport = async () => {
		...
		if (items.length === 0) {
			...
			if (blockedCount > 0 && blockedCount === preview.rows.length) {
				toast.error(
					'Todas las filas seleccionadas corresponden a productos serializados. Usa el módulo de equipos para moverlas.',
				);
				return;
			}
			...
			if (invalidCount > 0 && invalidCount === preview.rows.length) {
				toast.error(
					'Todas las filas tienen errores. Corrige la preview antes de importar.',
				);
				return;
			}
			...
			<ModalBody>
				<div className='grid max-h-[75vh] gap-4 overflow-y-auto overflow-x-hidden pr-1'>
					<div className='rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-4 text-sm text-neutral-600 dark:border-neutral-700 dark:bg-neutral-900/40 dark:text-neutral-300'>
						...
						<p className='mt-1'>
							Se intentan detectar columnas equivalentes a `product_id` y
							`quantity_change`.
						</p>
						<p className='mt-1'>
							Antes de importar puedes revisar y corregir cada fila detectada.
						</p>
					</div>
					<div>
						<input
						...
						{selectedFile && (
							<p className='mt-2 text-xs text-neutral-500'>{selectedFile.name}</p>
						)}
					</div>
					{preview && (
						<div className='min-w-0 overflow-hidden rounded-xl border border-neutral-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-900/40'>
							<div className='flex flex-wrap items-center justify-between gap-3'>
								<div>
									...
									<p className='mt-1 text-xs text-neutral-500'>
										...
										{preview.productIdHeader ?? 'no detectada'} · Ajuste ={' '}
										{preview.quantityHeader ?? 'no detectada'} · Tracking ={' '}
										{preview.serialTrackingHeader ?? 'no detectada'}
									</p>
								</div>
								<div className='flex flex-wrap gap-2'>
									...
									<span className='rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-medium text-rose-700 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-300'>
										{invalidPreviewRows.length - blockedPreviewRows.length}{' '}
										inválidas
									</span>
									<button
										type='button'
										...
										{showSourceColumns
											? 'Ocultar columnas CSV'
											: 'Mostrar columnas CSV'}
									</button>
								</div>
							</div>
							<div className='mt-4 rounded-lg border border-dashed border-neutral-200 bg-neutral-50 px-3 py-2 text-xs text-neutral-500 dark:border-neutral-700 dark:bg-neutral-900/30 dark:text-neutral-400'>
								Solo se importarán filas con `quantity_change` informado y distinto
								de `0`.
							</div>
							{validPreviewRows.length === 0 ? (
								<div className='mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300'>
									No hay filas listas para importar. Revisa los estados de la
									...
									<table className='min-w-full divide-y divide-neutral-200 dark:divide-neutral-800'>
										...
										<tbody className='divide-y divide-neutral-200 bg-white dark:divide-neutral-800 dark:bg-neutral-950/60'>
											{preview.rows.map((row) => {
												const rowState = getPreviewRowState(
													row,
													serialTrackedIdsSet,
												);
												return (
					...
					<div>
						...
						<Input
							id='inventory-import-reason'
							name='inventory-import-reason'
							value={reason}
			...
			<ModalFooter>
				<div className='ml-auto flex w-full flex-col-reverse gap-3 sm:w-auto sm:flex-row sm:items-center'>
					...
					<Button
						...
						isDisable={importButtonDisabled}>
