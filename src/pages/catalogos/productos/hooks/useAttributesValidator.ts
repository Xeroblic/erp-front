...
import { sanitiseAttributesInput, prepareAttributesForSubmit } from '../utils/dynamicAttributes.utils';
    ...
    const res = useMemo(() => {
        ...
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                ...
                if (parsed['cpu'] && typeof parsed['cpu'] === 'string' && !(parsed.cpu && parsed.cpu.model)) {
                    ensure(parsed, 'cpu.model', parsed['cpu']);
                }
                ...
                if (parsed['RAM'] && !(parsed.ram && (parsed.ram.capacity_gb || parsed.ram.gb))) {
                    ...
                    if (parsed.ram && parsed.ram.capacity_gb !== undefined && parsed.ram.gb === undefined) {
                ...
                if (parsed['storage'] && typeof parsed['storage'] === 'number' && !(parsed.storage && parsed.storage.primary)) {
        ...
        if (required.length > 0) {
            ...
            return { ok, missing, missingCount: missing.length, missingLabels, readyToPublish: ok } as any;
        }
        ...
        return { ok, missing: ok ? [] : ['attributes_json'], missingCount: ok ? 0 : 1, readyToPublish: ok };
