...
export const attributesSchema = Yup.object({
	cpu: Yup.object({
		...
		threads: Yup.object({
			...
			'threads-range',
			'threads.min debe ser <= threads.max',
			(v) => !v || v.min <= v.max,
		),
		base_clock_mhz: Yup.object({
			min: Yup.number().min(200).required(),
			max: Yup.number().min(200).required(),
		}).test('base-range', 'base.min <= base.max', (v) => !v || v.min <= v.max),
		boost_clock_mhz: Yup.object({
			min: Yup.number().min(200).required(),
			max: Yup.number().min(200).required(),
...
export const productSchema = Yup.object({
	...
	categories: Yup.array().of(
		Yup.object({ value: Yup.number().required(), label: Yup.string().required() }),
	),
...
export const productDetailSchema = Yup.object().shape({
	...
	category_ids: Yup.array().of(Yup.number()).min(1, 'Selecciona al menos una categoría'),
