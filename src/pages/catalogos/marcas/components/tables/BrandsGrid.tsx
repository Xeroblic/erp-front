...
const BrandsGrid: React.FC<BrandsGridProps> = ({ brands, loading, onView, onEdit, onDelete, onToggle }) => (
  <Card>
    <CardHeader>
            ...
            <div key={brand.id} className="rounded-lg border p-4 transition-shadow hover:shadow-md">
              <div className="mb-3 flex items-center space-x-3">
                ...
                <div className="flex-1">
                  ...
                  {brand.code && <p className="font-mono text-xs text-gray-500">{brand.code}</p>}
                  {brand.manufacturer && (
                    <p className="text-xs text-gray-500">Fabricante: {brand.manufacturer}</p>
                  )}
                </div>
              ...
              <div className="mt-4 flex items-center justify-between">
                <Button size="sm" variant="outline" onClick={() => onToggle(brand)}>
                  {brand.is_active ? 'Desactivar' : 'Activar'}
                </Button>
                <div className="flex space-x-2">
                  <Button size="sm" variant="outline" onClick={() => onView(brand)}>
