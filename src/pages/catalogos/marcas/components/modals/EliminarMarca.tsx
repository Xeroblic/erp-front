...
const EliminarMarca: React.FC<EliminarMarcaProps> = ({ isOpen, setIsOpen, brand, onConfirm }) => (
  <Modal isOpen={isOpen} setIsOpen={setIsOpen}>
          ...
          <div className='rounded-lg bg-red-50 p-4'>
            <div className='flex items-start'>
              ...
              <div className='space-y-2 text-sm text-red-700'>
                <p>
                  �Est�s seguro de que deseas eliminar la marca <strong>{brand.name}</strong>?
                </p>
                <p>
                  C�digo: <span className='font-mono font-semibold text-red-800'>{brand.code}</span>
                </p>
                <p>Fabricante: {brand.manufacturer}</p>
                {brand.products_count > 0 && (
                  <p className='font-semibold text-red-800'>
                    ?? Esta marca tiene {brand.products_count} productos asociados. Elimina o reasigna esos productos antes
                    de eliminar la marca.
                  </p>
                )}
              </div>
            </div>
    ...
    <ModalFooter>
      <div className='flex justify-end space-x-3'>
        ...
        <Button color='red' onClick={onConfirm} isDisable={!brand || brand.products_count > 0}>
          {brand && brand.products_count > 0 ? 'No se puede eliminar' : 'Eliminar Marca'}
        </Button>
      </div>
    </ModalFooter>
  </Modal>
