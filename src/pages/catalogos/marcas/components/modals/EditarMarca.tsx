...
const EditarMarca: React.FC<EditarMarcaProps> = ({ isOpen, setIsOpen, brand, onSubmit, isLoading }) => {
      ...
      <ModalHeader>
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Actualiza la informacion principal de la marca</p>
          </div>
        </div>
      </ModalHeader>
      <ModalBody>
        <form id="editBrandForm" className="space-y-4" onSubmit={onSubmit}>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <Input id="edit-brand-code" name="code" type="text" defaultValue={brand.code ?? ''} />
            </div>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">Si subes una nueva imagen se convertira automaticamente a WebP.</p>
          </div>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onChange={(event) => {
                  ...
                  setActive(value);
                  const hidden = document.getElementById('edit-brand-is-active') as HTMLInputElement | null;
                  if (hidden) hidden.value = value ? '1' : '0';
                }}
              />
              <Label htmlFor="edit-brand-active-toggle" className="!mb-0">Activa</Label>
            </div>
            <input id="edit-brand-is-active" name="is_active" type="hidden" value={active ? '1' : '0'} readOnly />
          </div>
        </form>
      </ModalBody>
      <ModalFooter>
