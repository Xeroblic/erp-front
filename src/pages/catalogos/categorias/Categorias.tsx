...
import { ICategory, ICategoryFilters } from './components/types';
...
const Categorias: React.FC = () => {
  ...
  const onView = (c: ICategory) => { setSelected(c); setViewOpen(true); };
  const onEdit = (c: ICategory) => { setSelected(c); setEditOpen(true); };
  const onDelete = (c: ICategory) => { setSelected(c); setDeleteOpen(true); };
    ...
    <PageWrapper name='categorias-admin'>
        ...
        <SubheaderLeft>
          <div className='flex items-center space-x-3'>
            <div className='flex h-10 w-10 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900/20'>
              <Icon icon='HeroSquares2X2' className='h-6 w-6 text-indigo-600 dark:text-indigo-400' />
            </div>
            <div>
              <h1 className='text-2xl font-bold text-gray-900 dark:text-white'>Categorías</h1>
              <p className='text-sm text-gray-600 dark:text-gray-400'>Organiza tus productos por categorías</p>
            </div>
          </div>
        </SubheaderLeft>
        <SubheaderRight>
          <Button color='blue' onClick={onCreate} icon='HeroPlus'>Nueva Categoría</Button>
        </SubheaderRight>
      </Subheader>
      ...
      <Container>
        {loading ? (
          ...
          <CategoriesTable categories={categories} onView={onView} onEdit={onEdit} onDelete={onDelete} />
        )}
      </Container>

      <CrearCategoria isOpen={createOpen} setIsOpen={setCreateOpen} onSubmit={handleCreateSubmit} categories={categories} />
      <EditarCategoria isOpen={editOpen} setIsOpen={setEditOpen} category={selected} onSubmit={handleEditSubmit} categories={categories} />
      <EliminarCategoria isOpen={deleteOpen} setIsOpen={setDeleteOpen} category={selected} onConfirm={handleConfirmDelete} />
