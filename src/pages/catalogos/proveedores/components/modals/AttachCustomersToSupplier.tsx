...
const AttachCustomersToSupplier: React.FC<Props> = ({ isOpen, onClose, onAttach }) => {
  ...
  return (
    <Modal isOpen={isOpen} setIsOpen={onClose} size='lg'>
      <ModalHeader>
        <h3 className='text-lg font-semibold text-gray-900 dark:text-white'>Asociar clientes al proveedor</h3>
      </ModalHeader>
      <ModalBody>
        <div className='space-y-4'>
          <div>
            <div className='mb-2 text-sm text-gray-700 dark:text-gray-300'>Selecciona uno o más clientes:</div>
            <SelectReact
              name='customers'
              isMulti
              isLoading={loading}
      ...
      <ModalFooter>
        <div className='flex justify-end space-x-2'>
          ...
          <Button
            ...
            onClick={async () => {
              const ids = selected.map((s) => Number(s.value)).filter((n) => !Number.isNaN(n));
              await onAttach(ids);
              setSelected([]);
              onClose();
            }}
