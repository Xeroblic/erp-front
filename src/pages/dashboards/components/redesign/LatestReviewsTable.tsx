...
const LatestReviewsTable: React.FC = () => {
	...
	const { items, itemsMeta, itemsLoading, itemsError } = useAppSelector(
		(state) => state.technicalReviews,
	);
				...
				<div className='flex items-center gap-2'>
					...
					<div>
						...
						<p className='text-xs text-gray-500'>
							Listado de ingresos técnicos recientes
						</p>
					</div>
				</div>
				<Button
			...
			<CardBody className='p-0'>
				{itemsError ? (
					<div className='flex flex-col items-center justify-center py-12 text-red-500'>
						...
						<p className='mt-2 text-xs text-gray-400'>{itemsError}</p>
					</div>
				) : items.length > 0 ? (
					<DataTable<IItem>
						columns={columns}
