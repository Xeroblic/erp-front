		...
		<Card className='group h-full border-zinc-100 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md dark:border-zinc-800 dark:shadow-zinc-900/10'>
			<CardBody className='flex flex-col justify-between p-6'>
				<div className='flex items-center justify-between'>
					{/* Icon Box */}
					<div className={`flex h-12 w-12 items-center justify-center rounded-2xl transition-transform group-hover:scale-110 ${colorClass}`}>
				...
				<div className='mt-6'>
					...
					<p className='mt-1 text-sm font-medium text-zinc-500 dark:text-zinc-400 capitalize'>
                        {title.toLowerCase()}
                    </p>
				</div>
				...
				{subtitle && (
					<div className='mt-4 flex items-center gap-2'>
                        {trend === 'up' && <Icon icon="HeroTrendingUp" className="h-4 w-4 text-emerald-500" />}
                        {trend === 'down' && <Icon icon="HeroTrendingDown" className="h-4 w-4 text-red-500" />}
