							...
							<PermissionGuard
								...
								<Link to={subPage.to} className='group block h-full'>
									<div className='flex h-full items-center justify-between rounded-xl border border-gray-100 bg-gray-50/80 p-3 transition-all duration-300 hover:border-blue-300 hover:bg-blue-50/50 hover:shadow-sm dark:border-gray-700/50 dark:bg-gray-900/40 dark:hover:border-blue-500/40 dark:hover:bg-gray-800'>
										<div className='flex items-center gap-3'>
											<div className='flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-white shadow-sm transition-transform duration-300 group-hover:scale-105 dark:bg-gray-800'>
												<Icon
													color={themeColor}
													icon={subPage.icon}
													className='text-xl transition-colors duration-300 group-hover:text-blue-600 dark:text-red-400 dark:group-hover:text-blue-400'
												/>
											</div>
										...
										<div className='flex h-6 w-6 flex-shrink-0 items-center justify-center text-gray-400 opacity-0 transition-all duration-300 group-hover:translate-x-1 group-hover:text-blue-600 group-hover:opacity-100 dark:group-hover:text-blue-400'>
											<svg
												xmlns='http://www.w3.org/2000/svg'
												width='16'
												height='16'
		...
		if (over && active.id !== over.id) {
			setSections((items) => {
				...
				sessionStorage.setItem(
					SESSION_ORDER_KEY,
					JSON.stringify(newOrder.map((i) => i.section)),
				);
				return newOrder;
			});
		}
	...
	return (
		<div className='w-full'>
			<DndContext
				...
				<div
					className={`grid w-full items-start gap-6 transition-all duration-500 ${getGridColsClass(columnCount)}`}>
					<SortableContext
						items={sections.map((s) => s.section)}
						strategy={rectSortingStrategy}>
						{sections.map((config, index) => {
							const section = (privatePages as Record<string, PageConfig>)[
								config.section
							];
							...
							if (config.items && config.items.length > 0) {
								subPagesToShow = config.items
									.map(
										(key) =>
											section.subPages![key] ||
											Object.values(section.subPages!).find(
												(sp) => sp.id === key,
											),
									)
							...
							return (
								<PermissionGuard
									...
									permission={section.authority}
									role={section.roles}>
									<SortableSectionCard
										id={config.section}
										section={section}
										subPagesToShow={subPagesToShow}
									/>
								</PermissionGuard>
							);
						})}
