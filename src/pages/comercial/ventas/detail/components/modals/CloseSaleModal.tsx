...
const CloseSaleModal: React.FC<Props> = ({
	...
	const handleConfirm = async () => {
		...
		const res = await dispatch(closeSaleThunk({ subsidiaryId, saleId, items: parsedPayload }));
		setIsSubmitting(false);
				...
				<div className='flex items-center gap-3'>
					...
					<div>
						...
						<p className='text-xs text-zinc-500 dark:text-zinc-400'>
							Ingresa las series solicitadas para cada ítem antes de confirmar el
							cierre.
						</p>
					</div>
				</div>
						...
						return (
							<div
								...
								<Textarea
									...
									onChange={(e) =>
										setSerialInputs((s) => ({ ...s, [it.id]: e.target.value }))
									}
								/>
							</div>
						);
