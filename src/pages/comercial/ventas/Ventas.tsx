...
const Ventas: React.FC = () => {
    ...
    try {
      await dispatch(cancelSale({
        ...
        data: {
          ...
          refund_amount: cancelForm.refund_amount ? parseFloat(cancelForm.refund_amount) : undefined,
  ...
  const getStatusBadge = (status: SaleStatus) => {
    const statusConfig: Record<SaleStatus, { color: 'yellow' | 'blue' | 'violet' | 'emerald' | 'red' | 'amber'; text: string }> = {
  ...
  const getPaymentStatusBadge = (paymentStatus: string) => {
    const statusConfig: Record<string, { color: 'yellow' | 'amber' | 'emerald' | 'red'; text: string }> = {
  ...
  const canShip = (sale: ISale) => {
    return sale.status === 'PAID' || (sale.status === 'INVOICED' && sale.payment_status === 'PAID');
  };
    ...
    <Container className="flex shrink-0 grow basis-auto flex-col pb-0">
          ...
          <CardBody>
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{formatCurrency(statistics.totalSalesAmount)}</p>
              </div>
            </div>
          </CardBody>
        </Card>
        ...
        <CardBody>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <Input
              ...
              onChange={(e) => setLocalFilters({ ...localFilters, sale_date_from: e.target.value })}
            />
            ...
            <Input
              ...
              onChange={(e) => setLocalFilters({ ...localFilters, sale_date_to: e.target.value })}
            />
      ...
      <Card>
        <CardHeader>
          <CardHeaderChild>
            <CardTitle>
              Lista de Ventas ({pagination.totalSales})
            </CardTitle>
          </CardHeaderChild>
        </CardHeader>
              ...
              {loading.fetch ? (
                ...
                sales.map((sale) => (
                  <Tr key={sale.id}>
                    ...
                    <Td className="font-mono">{sale.invoice_number || 'Sin factura'}</Td>
                    <Td className="font-mono">{formatCurrency(sale.total_amount)}</Td>
                    <Td>{sale.salesperson?.name || 'N/A'}</Td>
                    <Td>
                      <div className="flex flex-wrap gap-1">
                        <PermissionGuard permissions={[ERP_PERMISSIONS.SALES.VIEW]}>
                          <Button
                            size="sm"
                            variant="outline"
                            icon="HeroEye"
                        ...
                        {canInvoice(sale) && (
                          <PermissionGuard permissions={[ERP_PERMISSIONS.SALES.GENERATE_INVOICE]}>
                            <Button
                              size="sm"
                              color="indigo"
                              icon="HeroDocument"
                        ...
                        {canAddPayment(sale) && (
                          <PermissionGuard permissions={[ERP_PERMISSIONS.SALES.RECORD_PAYMENT]}>
                            <Button
                              size="sm"
                              color="green"
                              icon="HeroCreditCard"
                              onClick={() => {
                                ...
                                setPaymentForm({
                                  ...paymentForm,
                                  amount: (sale.total_amount - (sale.paid_amount || 0)).toString()
                                });
                        ...
                        {canShip(sale) && (
                          <PermissionGuard permissions={[ERP_PERMISSIONS.SALES.SHIP]}>
                            <Button
                              size="sm"
                              color="purple"
                              icon="HeroTruck"
                        ...
                        {sale.invoice_number && (
                          <Button
                            ...
                            onClick={() => {
                              window.open(`/api/sales/${sale.id}/invoice-pdf`, '_blank');
                            }}
                          />
                        )}
                        
                        {canCancel(sale) && (
                          <PermissionGuard permissions={[ERP_PERMISSIONS.SALES.CANCEL]}>
                            <Button
                              size="sm"
                              color="red"
                              icon="HeroXMark"
          ...
          {selectedSale && (
            <div className="space-y-4">
              <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <p><strong>Cliente:</strong> {selectedSale.customer?.name}</p>
                <p><strong>Total:</strong> {formatCurrency(selectedSale.total_amount)}</p>
                <p><strong>Pagado:</strong> {formatCurrency(selectedSale.paid_amount || 0)}</p>
                <p><strong>Pendiente:</strong> {formatCurrency(selectedSale.total_amount - (selectedSale.paid_amount || 0))}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setShipForm({...shipForm, shipped_date: e.target.value})}
                  />
                </div>
                
                <div>
                  <label className="block text-sm font-medium mb-1">Entrega Estimada</label>
                  <Input
                    type="date"
                    value={shipForm.expected_delivery_date}
                    onChange={(e) => setShipForm({...shipForm, expected_delivery_date: e.target.value})}
                  />
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-medium mb-1">Notas de Envío</label>
                <Textarea
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setCancelForm({...cancelForm, refund_amount: e.target.value})}
                  placeholder="Opcional - máximo: " + selectedSale.total_amount
                />
              </div>
