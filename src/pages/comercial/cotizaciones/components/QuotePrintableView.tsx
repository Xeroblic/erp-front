...
const QuotePrintableView: React.FC<QuotePrintableViewProps> = ({ quote }) => {
	...
	return (
		<div className='mx-auto min-h-[229mm] max-w-[216mm] bg-white p-10 font-sans text-[9px] leading-snug text-gray-800 shadow-lg'>
			{/* Header */}
			<div className='mb-5 flex items-start justify-between'>
				<div className='w-[60%]'>
					...
					{company.logoUrl && (
						<p className='mb-0.5 text-sm font-bold uppercase text-gray-900'>
							{company.name}
						</p>
					)}
					<p className='text-[9px] text-gray-700'>Giro: {company.activity}</p>
					...
					{company.website && (
						<p className='text-[9px] text-gray-700'>Web: {company.website}</p>
					)}
				</div>

				<div className='flex w-[36%] flex-col items-center'>
					<div className='w-full border-2 border-rose-600 py-2 text-center'>
						<h2 className='text-xs font-bold text-rose-600'>R.U.T.: {company.rut}</h2>
						<div className='my-1 w-full bg-rose-50 py-0.5'>
							<h3 className='text-xs font-bold uppercase text-rose-600'>
								COTIZACIÓN
							</h3>
						</div>
						<h4 className='text-sm font-bold text-gray-900'>N° {quote.id}</h4>
					</div>
					<p className='mt-1.5 text-[10px] font-bold text-gray-900'>
			...
			{/* Cliente */}
			<div className='mb-5 border border-gray-300 bg-gray-50 p-2'>
			...
			<div className='mb-2.5'>
				{/* Header */}
				<div className='flex border-b border-t border-gray-300 bg-gray-100 py-1.5 text-[9px] font-bold'>
				...
				{items.map((item, idx) => {
					...
					return (
						<div key={idx} className='flex border-b border-gray-200 py-1.5 text-[9px]'>
							<div className='w-[8%] text-center'>{quantity}</div>
							<div className='w-[17%] pl-1'>{sku}</div>
							<div className='w-[45%] pl-1'>
								<p className='font-bold'>{name}</p>
								{detail && (
									<p className='mt-0.5 text-[7px] text-gray-400'>{detail}</p>
								)}
							</div>
			...
			<div className='mb-5 flex justify-end'>
				<div className='w-[180px]'>
					...
					<div className='mt-1 flex justify-between border-t border-black pt-1'>
			...
			<div className='border-t border-gray-200 pt-2.5'>
				<div className='mb-2.5 flex gap-5'>
					<div className='flex-1'>
						...
						<p className='mb-0.5 text-[8px] text-gray-600'>
							• Validez Oferta:{' '}
						...
						{quote.notes && (
							<div className='mt-1.5'>
								...
								<p className='text-[8px] text-gray-600'>{quote.notes}</p>
							</div>
						)}
					</div>
						...
						{company.bankInfo.length > 0 && (
							<div>
								...
								{company.bankInfo.map((info: string, idx: number) => (
									<p key={idx} className='mb-0.5 text-[8px] text-gray-600'>
										{info}
									</p>
								))}
							</div>
