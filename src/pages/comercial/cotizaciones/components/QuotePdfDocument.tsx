...
const getCompanyInfo = (quote: IQuote) => {
    ...
    const activity = meta.activity || mainCompany?.business_activity || 'Venta de Artículos Computacionales';

    // Dirección Inteligente
    let addressBase = meta.address || activeSub?.subsidiary_address || mainCompany?.company_address || '';
    let communeName = meta.commune || (activeSub?.commune as any)?.name || '';
    
    const fullAddress = communeName && !addressBase.includes(communeName)
    ...
    // Datos Bancarios
    const bankData = (meta.bank_info as any) ?? (activeSub as any)?.bank_info ?? (mainCompany as any)?.bank_info;
    ...
    // Otros metadatos
    const allowedPaymentMethods = meta.allowed_payment_methods || activeSub?.subsidiary_allowed_payment_methods || meta.payment_methods || [];
    const deliveryTerm = meta.delivery_term || activeSub?.subsidiary_delivery_term;
    const quoteValidityText = meta.quote_validity_text || activeSub?.subsidiary_quote_validity_text || '';
    const quoteValidityDays = meta.quote_validity_days || activeSub?.subsidiary_quote_validity_days || null;
                ...
                <View style={styles.mainContent}>
                    ...
                    <View style={styles.clientSection}>
                        <View style={styles.clientRow}>
                            ...
                            <Text style={[styles.clientValue, styles.boldText, { textTransform: 'uppercase' }]}>
                                {customer.razon_social || customer.name || 'Cliente General'}
                            </Text>
                            <Text style={[styles.clientLabel, { width: 30, textAlign: 'right' }]}>RUT:</Text>
                        ...
                        <View style={styles.clientRow}>
                            ...
                            <Text style={[styles.clientLabel, { width: 40, textAlign: 'right' }]}>Fono:</Text>
                    ...
                    <View style={styles.twoColumnFooter}>
                        ...
                        <View style={styles.footerCol}>
                            ...
                            <Text style={styles.noteText}>
                                • Validez Oferta: {company.quoteValidityText || (company.quoteValidityDays ? `${company.quoteValidityDays} días hábiles` : '7 días hábiles')}
                            </Text>
                            <Text style={styles.noteText}>
                                • Forma de Pago: {(company.allowedPaymentMethods as any)?.length ? (company.allowedPaymentMethods as any).join(' / ') : 'Contado / Transferencia / WebPay'}
                            </Text>
                            <Text style={styles.noteText}>
                                • Entrega: {company.deliveryTerm || 'A convenir o retiro en tienda.'}
                            </Text>
                            {company.commercialTerms ? <Text style={styles.noteText}>• {company.commercialTerms}</Text> : null}
                            ...
                            {company.bankInfo.length > 0 && (
                                <View>
                                    ...
                                    {company.bankInfo.map((info: string, i: number) => (
                                        <Text key={i} style={styles.noteText}>{info}</Text>
                                    ))}
                                </View>
                            )}
                        </View>
