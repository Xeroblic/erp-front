...
export const quotationSchema = Yup.object().shape({
	...
	terms_conditions: Yup.array().nullable().of(Yup.mixed()),

	// notes: nullable, string
	notes: Yup.string().nullable().max(1000, 'Las notas no pueden exceder 1000 caracteres'),
	...
	items: Yup.array()
		...
		.test('items-valid', 'Todos los items deben tener precio y cantidad validos', (items) => {
			...
			return items.every((item: any) => {
				...
				const hasValidName =
					item.type === 'custom' ? item.customer_name?.trim().length > 0 : true;
...
export const statusOptions: TSelectOptions = quoteStatusOptions.map(
