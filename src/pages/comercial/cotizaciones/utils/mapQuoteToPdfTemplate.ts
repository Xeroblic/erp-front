...
export function mapQuoteToPdfTemplate(quote: IQuote) {
	...
	// ========== COMPANY INFO ==========
	const metaCompany =
		quote.metadata?.company ?? quote.metadata?.company_info ?? quote.metadata?.issuer ?? null;
	...
	const companyTemplate = {
		...
		addresses:
			metaCompany?.addresses ?? metaCompany?.address_lines ?? metaCompany?.address ?? [],
		bankInfo:
	...
	const customerInfo = {
		...
		giro: c?.giro ?? c?.business ?? '—',
	};

	// ========== VENDEDOR ==========
	const sp = (quote.metadata as any)?.seller ?? (quote.metadata as any)?.salesperson ?? {};
	...
	const sellerInfo = {
		...
		email: sp?.email ?? (quote as any)?.salesperson_email ?? '—',

		phone: sp?.phone ?? sp?.mobile ?? (quote as any)?.salesperson_phone ?? '—',
	};
	...
	const mappedItems = items.map((item) => ({
		...
		descripcion: item.customer_name ?? item.product?.name ?? 'Ítem sin nombre',
		garantia: item.metadata?.warranty ?? (item as any).warranty ?? '—',
	...
	const taxAmount =
		toNumber(quote.totals?.tax_amount) ?? toNumber(quote.tax_amount) ?? totalNet * taxRate;
	...
	if (quote.internal_notes) observationLines.push(`Notas internas: ${quote.internal_notes}`);
	...
	return {
		...
		clientInfo: {
			...
			...customerInfo,
			paymentCondition: quote.payment_method ?? '—',
			deliveryCondition:
				quote.metadata?.delivery_method ?? quote.metadata?.delivery_terms ?? '—',
			seller: sellerInfo,
		},
