...
const sanitize = (s?: string | null) => (s ? String(s) : '')
  .replace(/\n|\r|\t/g, ' ')
  .replace(/\s+/g, ' ')
  .trim();
...
const NotificationSwipeItem: React.FC<Props> = ({ n, onRead, onUnread, onArchive, onDelete, onOpen, archiveLabel = 'Archivar' }) => {
        ...
        <div className='absolute inset-0 overflow-hidden' style={{ opacity: Math.abs(dx) > 1 ? 1 : 0 }}>
          ...
          <div className='pointer-events-none absolute inset-0 flex items-center justify-between px-4'>
            <div className='flex items-center gap-2 text-emerald-600'>
              ...
              <span className='hidden text-sm sm:inline'>{n.status !== 'read' ? 'Marcar leída' : 'Leída'}</span>
            </div>
            <div className='flex items-center gap-2 text-amber-600'>
              <Icon icon={archiveLabel.toLowerCase().includes('des') ? 'HeroArchiveBoxXMark' : 'HeroArchiveBox'} className='h-5 w-5' />
        ...
        <div className='grid grid-cols-[auto_1fr_auto] items-start gap-3'>
          ...
          <div>
            <div className='flex items-center gap-2'>
              ...
              {n.bucket === 'Important' && (
                <span className='rounded-full bg-rose-100 px-2 py-0.5 text-xs text-rose-700'>Importante</span>
              )}
            </div>
            <div className='text-sm text-zinc-700 dark:text-zinc-300'>{message}</div>
            <div className='mt-2 flex items-center gap-3'>
              {n.status !== 'read' ? (
                <span className='rounded-full bg-emerald-100 px-2 py-0.5 text-xs text-emerald-700'>No leída</span>
              ) : (
                <span className='rounded-full bg-zinc-100 px-2 py-0.5 text-xs text-zinc-700'>Leída</span>
              )}
              <button
