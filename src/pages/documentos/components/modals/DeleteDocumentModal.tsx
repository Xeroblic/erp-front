...
const DeleteDocumentModal: React.FC<DeleteDocumentModalProps> = ({ isOpen, setIsOpen, document, onConfirm }) => (
      ...
      {document ? (
        <div className='space-y-4 text-sm'>
          ...
          <div className='flex items-center space-x-2 text-gray-500'>
            <span>Ruta:</span>
            <Badge variant='outline'>{document.file_path}</Badge>
          </div>
          <div className='rounded-md border border-red-200 bg-red-50 p-3 text-red-700'>
            <div className='flex items-start'>
              ...
              <div>
                ...
                <p>El archivo se eliminará de forma permanente y no podrás recuperarlo.</p>
              </div>
            </div>
          </div>
        </div>
      ) : (
        <div className='py-6 text-center text-sm text-gray-500'>Selecciona un documento para eliminar.</div>
      )}
    </ModalBody>
    <ModalFooter>
      <div className='flex justify-end space-x-3'>
        ...
        <Button color='red' onClick={onConfirm} isDisable={!document}>
          Eliminar documento
        </Button>
      </div>
    </ModalFooter>
