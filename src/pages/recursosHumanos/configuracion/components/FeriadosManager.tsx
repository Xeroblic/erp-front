...
const FeriadosManager: React.FC = () => {
	...
	const [apiHolidays, setApiHolidays] = useState<
		{ date: string; title: string; type?: string; inalienable?: boolean; extra?: string }[]
	>([]);
					...
					<div className='mt-8'>
						{loadingApi ? (
							...
							<div className='flex flex-col gap-6'>
								<div className='h-auto w-full overflow-x-auto rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900/40'>
								...
								{holidays.length > 0 && (
									<div className='mt-4 overflow-x-auto'>
										<h3 className='mb-3 text-lg font-semibold text-zinc-300'>
											Feriados Agregados Manualmente
										</h3>
				...
				<ModalBody>
					{selectedEvent && (
						<div className='flex flex-col gap-4 text-zinc-600 dark:text-zinc-300'>
							<div className='rounded-lg border border-zinc-200 bg-zinc-100 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/50'>
							...
							<div className='grid grid-cols-2 gap-4'>
								<div className='rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-transparent dark:bg-zinc-900/50'>
									...
									<div className='font-semibold'>
										{selectedEvent.isManual ? (
											<span className='flex items-center gap-1 text-yellow-600 dark:text-yellow-400'>
												<Icon icon='HeroUser' size='text-sm' /> Manual
											</span>
										) : (
											<span className='flex items-center gap-1 text-blue-600 dark:text-blue-400'>
												<Icon icon='HeroGlobeAlt' size='text-sm' /> Oficial
												(API)
											</span>
										)}
									</div>
								...
								{selectedEvent.extra && (
									<div className='col-span-2 rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-transparent dark:bg-zinc-900/50'>
										<div className='mb-1 text-xs text-zinc-500 dark:text-zinc-500'>
											Información Extra
										</div>
										<div className='font-semibold text-zinc-800 dark:text-zinc-200'>
							...
							<div className='mt-2 flex gap-2'>
								{selectedEvent.inalienable && (
									<span className='rounded-full border border-red-200 bg-red-100 px-3 py-1 text-xs font-semibold text-red-600 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-400'>
										Irrenunciable
									</span>
								)}
								{selectedEvent.recurring && (
									<span className='rounded-full border border-emerald-200 bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-600 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-400'>
										Recurrente
									</span>
								)}
							</div>
