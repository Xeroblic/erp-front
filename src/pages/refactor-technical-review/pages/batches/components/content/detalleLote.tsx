...
export interface DetalleLoteProps {
	...
	activeTab: 'all' | 'notebook' | 'desktop' | 'aio' | 'docking' | 'monitor';
	setActiveTab: (tab: 'all' | 'notebook' | 'desktop' | 'aio' | 'docking' | 'monitor') => void;
}
			...
			<div className='grid gap-4 sm:grid-cols-2'>
				...
				<Card className='border-l-4 border-purple-500 dark:border-purple-400'>
					<CardBody className='p-5'>
						...
						{batch.warehouse_id && (
							<p className='mt-1 text-xs text-zinc-400'>ID: {batch.warehouse_id}</p>
						)}
					</CardBody>
				</Card>
			</div>
				...
				<Card>
					...
					<CardBody>
						<div className='grid grid-cols-2 gap-3 md:grid-cols-5'>
							{/* Botón "Todos" explícito o implícito? 
                                El usuario pidió que la distribución afecte la tabla.
                                Si agregamos un card "Todos"? No, el diseño actual es grid de tipos.
                                Mejor permitir deseleccionar.
                            */}
								...
								([type, count]) => (
									<div
										...
										<div
											...
											<Icon
												icon={
													equipmentIconMap[type] || 'HeroComputerDesktop'
												}
												className='h-5 w-5'
											/>
										</div>
					...
					<div className='grid gap-4 sm:grid-cols-3'>
						...
						<div className='flex items-center gap-3'>
							...
							<div>
								...
								<p className='text-sm font-medium text-zinc-800 dark:text-zinc-200'>
									{batch.updated_at
										? new Date(batch.updated_at).toLocaleDateString('es-CL', {
												year: 'numeric',
												month: 'short',
												day: 'numeric',
												hour: '2-digit',
