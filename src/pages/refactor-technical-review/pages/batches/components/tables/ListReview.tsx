...
const ListReview: React.FC<ListReviewProps> = ({ batchId, activeTab }) => {
	...
	const filteredItems = useMemo(() => {
		if (batchId === 17 && reviewStatusFilter === 'sold') {
			return items.filter((item) => serialNumbers.includes(item.serial_number));
		}
		return items;
	}, [items, batchId, reviewStatusFilter]);
							...
							<div className='flex flex-col'>
								...
								{isSold && (
									<Badge
										variant='outline'
										color='emerald'
										className='gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-bold'>
	...
	return (
		<Card>
			<CardBody>
				{/* ═══ Progress Header (Only for Batch 17 + 'sold' filter) ═══ */}
				{batchId === 17 && reviewStatusFilter === 'sold' && (
					<div className='mb-8 rounded-2xl border border-zinc-200 bg-zinc-50/50 p-6 dark:border-zinc-700/50 dark:bg-zinc-800/20'>
						<div className='mb-6 flex flex-col items-center justify-between gap-4 sm:flex-row'>
							<div className='flex items-center gap-4'>
								<div className='flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-100 text-emerald-600 dark:bg-emerald-900/40 dark:text-emerald-400'>
									<Icon icon='HeroCheckBadge' className='h-7 w-7' />
								</div>
								<div>
									<h3 className='text-lg font-bold text-zinc-900 dark:text-zinc-100'>
										Seguimiento Venta Canal 13
									</h3>
									<p className='text-sm text-zinc-500'>
										Equipos revisados de la lista de 143 especiales.
									</p>
								</div>
							</div>
						...
						{/* Progress Bar */}
						<div className='mb-8 h-3 w-full overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700'>
							<div
