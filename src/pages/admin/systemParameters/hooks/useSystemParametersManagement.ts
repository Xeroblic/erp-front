...
export const useSystemParametersManagement = () => {
	...
	const applyFilters = useCallback(
		(parameters: SystemParameter[], filters: SystemParameterFilters) => {
			return parameters.filter((param) => {
				if (
					filters.search &&
					!param.key.toLowerCase().includes(filters.search.toLowerCase()) &&
				...
				if (
					...
					param.is_editable !== filters.is_editable
				) {
					return false;
				}
	...
	const handleFilterChange = useCallback(
		(newFilters: Partial<SystemParameterFilters>) => {
	...
	const handleCreateParameter = useCallback(
		async (data: SystemParameterCreate) => {
	...
	const handleUpdateParameter = useCallback(
		async (id: number, data: SystemParameterUpdate) => {
	...
	const refreshParameters = useCallback(async () => {
		...
		try {
			...
			setState((prev) => ({
				...prev,
				isLoading: false,
				error: 'Error al cargar parámetros',
			}));
		}
