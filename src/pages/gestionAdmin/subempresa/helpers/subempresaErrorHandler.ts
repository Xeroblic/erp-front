...
export const handleSubempresaError = (error: any, isEditing: boolean) => {
	...
	if (isDuplicate) {
		if (
			...
			toast.error(
				'Este gerente ya está asignado a otra subempresa. Seleccione otro gerente.',
			);
		} else if (
			errorMessage.includes('email') ||
			fullError.includes('subsidiary_email') ||
			fullError.includes('email')
		) {
			toast.error('El email ingresado ya está registrado en otra subempresa');
		} else if (
			errorMessage.includes('rut') ||
			fullError.includes('subsidiary_rut') ||
			fullError.includes('rut')
		) {
