...
const TableUser: React.FC<Props> = ({ tableData, status, error, globalFilter, setGlobalFilter }) => {
        ...
        () => [
            ...
            columnHelper.display({
                ...
                cell: (info) => {
                    ...
                    if (!roles.length) return <span className='text-zinc-400'>Sin roles asignados</span>;
                    return (
                    ...
                    return (
                        <div className='text-center'>
                            ...
                            <div className='text-xs text-zinc-500'>
                                {user.directPermissionsCount} directo{user.directPermissionsCount === 1 ? '' : 's'}
                            </div>
                        </div>
                    );
                },
            ...
            columnHelper.accessor('is_active', {
                ...
                cell: (info) => (
                    <Badge color={info.getValue() ? 'emerald' : 'red'} className='inline-flex items-center gap-1'>
                        <div className={`h-2 w-2 rounded-full ${info.getValue() ? 'bg-emerald-300' : 'bg-red-300'}`} />
            ...
            columnHelper.display({
                ...
                cell: (info) => (
                    <div className='flex gap-2'>
                        <Button variant='outline' size='sm' color='amber' onClick={() => handleManageUser(info.row.original.id)}>
                            <Icon icon='HeroCog6Tooth' className='mr-1' />
                            Gestionar
                        </Button>
                        <Button variant='outline' size='sm' color={info.row.original.is_active ? 'red' : 'emerald'}>
                            <Icon icon={info.row.original.is_active ? 'HeroXCircle' : 'HeroCheckCircle'} className='mr-1' />
            ...
            <Card className='overflow-hidden'>
                <CardBody className='p-0'>
                    <div className='flex items-center justify-between border-b border-zinc-700 p-6'>
                        ...
                        <p className='text-xs text-zinc-500'>Última actualización: {new Date().toLocaleDateString('es-ES')}</p>
                    </div>

                    <div className='overflow-x-auto'>
                        {status === 'loading' && <div className='py-12 text-center text-zinc-400'>Cargando usuarios...</div>}
