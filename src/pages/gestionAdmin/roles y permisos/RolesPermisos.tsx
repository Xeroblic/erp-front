...
const RolesPermisos: React.FC = () => {
	...
	const formik = useFormik({
		...
		onSubmit: (values) => {
			if (editingUser) {
				dispatch(updateUsuarioRolesPerms({ id: editingUser.id, ...values })).then(() => {
					dispatch(fetchUsuariosConRolesPerms());
					setEditingUser(null);
				});
			}
				...
				<ModalBody>
					<form onSubmit={formik.handleSubmit} className='space-y-4'>
						...
						<SelectReact
							...
							isMulti
							options={[]}
							value={formik.values.roles.map((slug) => ({
								label: slug,
								value: slug,
							}))}
							onChange={(newValue) =>
						...
						{formik.touched.roles && formik.errors.roles && (
							<div className='text-sm text-red-600'>{formik.errors.roles}</div>
						)}
						...
						<SelectReact
							...
							isMulti
							options={[]}
							value={formik.values.permisos.map((clave) => ({
								label: clave,
								value: clave,
							}))}
							onChange={(newValue) => {
