	...
	const filteredResults = useMemo(() => {
		...
		return results.filter((r: any) => {
			...
			if (branchId && branchId > 0) {
				...
				const numericBranch =
					typeof rBranch === 'string' ? Number(rBranch) : Number(rBranch || 0);
				if (numericBranch !== branchId) return false;
			}

			if (customerText) {
				const rCustomerId = r?.customer_id || r?.customerId || r?.cliente_id;
				const rCustomerName = (r?.customer_name || r?.customer || r?.cliente || '')
					.toString()
					.toLowerCase();
				if (customerId) {
					const numericId =
						typeof rCustomerId === 'string'
							? Number(rCustomerId)
							: Number(rCustomerId || 0);
