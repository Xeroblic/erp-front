...
export const buildYearlyAnalytics = (filteredResults: SaleRecord[]): PdfContent => {
	...
	const chartStack = sortedYears.map(year => {
		...
		return {
			columns: [
				{ text: String(year), width: 60, fontSize: 9, color: '#475569', alignment: 'right', margin: [0, 4, 8, 0] },
				{
					stack: [
						{
							canvas: [
								{ type: 'rect', x: 0, y: 0, w: (barWidth / 100) * 340, h: 16, r: 4, color: '#38bdf8' }
							]
						}
					],
					width: 340,
				...
				{ text: currencyFormatter.format(total), width: '*', fontSize: 9, bold: true, color: '#0f172a', margin: [8, 4, 0, 0] }
			]
		};
	});
	...
	return {
		stack: [
			{ text: 'ANÁLISIS MACRO: COMPARATIVA AÑO CONTRA AÑO (YoY)', fontSize: 16, bold: true, color: '#0f172a', margin: [0, 0, 0, 16] },
			{
				table: {
					headerRows: 1,
					widths: ['*', 'auto', 'auto'],
					body: [
						[
							{ text: 'AÑO', fontSize: 9, bold: true, color: '#ffffff', fillColor: '#0f172a', margin: [4, 6, 4, 6] },
							{ text: 'VENTAS TOTALES', fontSize: 9, bold: true, color: '#ffffff', fillColor: '#0f172a', alignment: 'right', margin: [4, 6, 4, 6] },
							{ text: 'CRECIMIENTO (YoY)', fontSize: 9, bold: true, color: '#ffffff', fillColor: '#0f172a', alignment: 'right', margin: [4, 6, 4, 6] },
						],
						...sortedYears.map((year, idx) => {
							...
							if (previous && previous.total > 0) {
								const yoy = ((current.total - previous.total) / previous.total) * 100;
							...
							return [
								{ text: String(year), fontSize: 10, color: '#334155', bold: true, fillColor, margin: [4, 6, 4, 6], border: [false, false, false, false] },
								{ text: currencyFormatter.format(current.total), fontSize: 10, bold: true, color: '#0f172a', alignment: 'right', fillColor, margin: [4, 6, 4, 6], border: [false, false, false, false] },
								{ text: yoyText, fontSize: 10, bold: true, color: yoyColor, alignment: 'right', fillColor, margin: [4, 6, 4, 6], border: [false, false, false, false] },
							];
						})
					]
				},
			...
			{ text: 'VOLUMEN HISTÓRICO ANUAL', fontSize: 12, bold: true, color: '#0f172a', margin: [0, 0, 0, 16] },
			{ stack: chartStack }
		],
		pageBreak: 'before'
	} as PdfContent;
...
export const buildSemesterDeepDive = (filteredResults: SaleRecord[]): PdfContent[] => {
	...
	const semesters: Record<string, { year: number; semester: 1 | 2; total: number; count: number; customers: Record<string, number>; months: Record<number, number> }> = {};
	...
	filteredResults.forEach(r => {
		...
		if (!semesters[semKey]) semesters[semKey] = { year, semester, total: 0, count: 0, customers: {}, months: {} };
	...
	return sortedKeys.map(key => {
		...
		const cardW = 158;
		const cardH = 75;
		const createCard = (title: string, value: string, bgColor: string, txtColor: string = 'white', valColor: string = 'white') => ({
			stack: [
				{
					canvas: [
						{ type: 'rect', x: 0, y: 0, w: cardW, h: cardH, r: 6, color: bgColor },
				...
				{
					stack: [
						{ text: title.toUpperCase(), fontSize: 8, bold: true, color: txtColor, margin: [0, 0, 0, 8], letterSpacing: 0.5 },
			...
			if (val > 0 && current.total > 0) {
				...
				if (sliceAngle > 0.01) {
					...
					for (let step = 0; step <= steps; step++) {
						...
						points.push({ x: pieCx + pieRadius * Math.cos(t), y: pieCy + pieRadius * Math.sin(t) });
					}
					pieCanvas.push({ type: 'polyline', lineWidth: 1, lineColor: '#ffffff', color, closePath: true, points });
					currentAngle += sliceAngle;
				} else {
					pieCanvas.push({ type: 'ellipse', x: pieCx + (pieRadius/2)*Math.cos(currentAngle), y: pieCy + (pieRadius/2)*Math.sin(currentAngle), r1: 2, r2: 2, color });
				}
			} else if (val > 0 && current.total === 0) {
				pieCanvas.push({ type: 'ellipse', x: pieCx, y: pieCy, r1: pieRadius, r2: pieRadius, color });
			}
			
			if (val > 0) {
				legendBody.push([
					{ canvas: [{ type: 'ellipse', x: 4, y: 7, r1: 4, r2: 4, color }] },
					{ text: monthNames[m].toUpperCase(), fontSize: 9, bold: true, color: '#475569', margin: [6, 4, 0, 4] },
					{ text: currencyFormatter.format(val), fontSize: 9, bold: true, color: '#0f172a', alignment: 'right', margin: [0, 4, 0, 4] },
					{ text: current.total > 0 ? ((val / current.total) * 100).toFixed(1) + '%' : '0%', fontSize: 9, color: '#64748b', alignment: 'right', margin: [0, 4, 0, 4] },
				]);
			}
		});

		if (pieCanvas.length === 0) {
			pieCanvas.push({ type: 'ellipse', x: pieCx, y: pieCy, r1: pieRadius, r2: pieRadius, color: '#f8fafc' });
			legendBody.push([{ text: 'Sin movimientos mensuales', colSpan: 4, fontSize: 9, color: '#94a3b8', margin: [0, 4, 0, 4], alignment: 'center' }, {}, {}, {}]);
		}

		return {
			stack: [
				{ text: `RENDIMIENTO SEMESTRAL: ${current.year} - S${current.semester}`, fontSize: 18, bold: true, color: '#0f172a', margin: [0, 0, 0, 20] },
				{
					columns: [
						createCard('Ventas Totales', currencyFormatter.format(current.total), '#1e293b', '#94a3b8', '#ffffff'),
						{ width: 10, text: '' },
						createCard('Órdenes', String(current.count), '#f1f5f9', '#64748b', '#0f172a'),
						{ width: 10, text: '' },
						createCard('Ticket Promedio', currencyFormatter.format(avgTicket), '#f1f5f9', '#64748b', '#0f172a'),
					],
					margin: [0, 0, 0, 15]
				},
				{
					columns: [
						createCard(`Crecimiento vs ${current.year - 1}`, yoyText, '#ffffff', '#64748b', yoyColor),
					],
					margin: [0, 0, 0, 25]
				},
				{
					columns: [
						{
							stack: [
								{ text: 'DISTRIBUCIÓN MENSUAL', fontSize: 11, bold: true, color: '#334155', margin: [0, 0, 0, 16] },
						...
						{
							stack: [
								{ text: 'DESGLOSE ACUMULADO', fontSize: 11, bold: true, color: '#334155', margin: [0, 0, 0, 12] },
								{
									table: {
					...
					table: {
						...
						body: [
							[
								{
									stack: [
										{ text: '✧ SÍNTESIS ANALÍTICA', fontSize: 10, bold: true, color: '#38bdf8', margin: [0, 0, 0, 8], letterSpacing: 1 },
										{ text: calloutText, fontSize: 10, color: '#e2e8f0', lineHeight: 1.4 }
									],
									fillColor: '#1e293b',
									margin: [16, 16, 16, 16],
				...
				{ text: `TOP 3 CLIENTES DEL SEMESTRE`, fontSize: 12, bold: true, color: '#0f172a', margin: [0, 0, 0, 12] },
				{
					table: {
						headerRows: 1,
						widths: ['auto', '*', 'auto'],
						body: [
							...
							...topCustomers.map((c, idx) => {
                                ...
                                const fillColor = isEven ? '#fafafa' : '#ffffff';
                                return [
                                    { text: `#${idx + 1}`, fontSize: 10, color: '#64748b', bold: true, fillColor, border: [false, false, false, false], margin: [4, 8, 4, 8] },
                                    { text: c.name.toUpperCase(), fontSize: 10, color: '#1e293b', bold: true, fillColor, border: [false, false, false, false], margin: [4, 8, 4, 8] },
                                    { text: currencyFormatter.format(c.total), fontSize: 10, color: '#10b981', bold: true, alignment: 'right', fillColor, border: [false, false, false, false], margin: [4, 8, 4, 8] },
                                ];
                            }),
                            ...(topCustomers.length === 0 ? [[
                                { text: '-', fontSize: 10, color: '#94a3b8', border: [false, false, false, false], margin: [4, 8, 4, 8] },
                                { text: 'Sin clientes en este periodo', fontSize: 10, color: '#94a3b8', border: [false, false, false, false], margin: [4, 8, 4, 8] },
                                { text: '-', fontSize: 10, color: '#94a3b8', alignment: 'right', border: [false, false, false, false], margin: [4, 8, 4, 8] }
                            ]] : [])
						],
					},
					layout: {
...
export const buildRiskAnalytics = (filteredResults: SaleRecord[]): PdfContent => {
	...
	filteredResults.forEach(r => {
		...
		let pStatusRaw = String((r as any).payment_status || (r as any).estado_pago || '').toLowerCase().trim();
		...
		const isRefund = ['refunded', 'returned', 'anulado', 'cancelled', 'canceled', 'cancelado', 'failed', 'devuelto', 'reembolsado', 'rechazado'].includes(statusRaw) || 
						 ['refunded', 'returned', 'failed'].includes(pStatusRaw) || 
						 amount < 0;
						 
		const isSuccess = ['completed', 'processing', 'paid', 'pagado'].includes(statusRaw) || 
