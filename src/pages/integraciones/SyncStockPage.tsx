...
const SyncStockPage: React.FC = () => {
	...
	const wooIntegrations = useMemo(
		() =>
			integrations
				.filter(
					(integration) =>
						integration.provider === 'woocommerce' && integration.is_active,
				)
				.sort((a, b) => a.name.localeCompare(b.name)),
		[integrations],
	);

	const selectedIntegration = wooIntegrations.find(
	...
	const handleSyncStock = async () => {
		...
		try {
			...
			toast.success(
				response.message ||
					`Stock sincronizado: ${response.result?.synced || 0} productos actualizados`,
			);
		} catch (error: any) {
			...
			<Container>
				...
				{!loading && wooIntegrations.length === 0 && (
					<Alert
						...
						variant='outline'
						className='mb-4'>
						No hay integraciones de WooCommerce activas. Crea una integración en modo
						Lectura/Escritura para usar esta herramienta.
					</Alert>
				)}
						...
						<div className='space-y-6'>
								...
								{selectedIntegration && (
									<div className='mt-2 flex flex-wrap items-center gap-2 text-sm text-gray-500'>
										<Badge color='blue' variant='outline'>
											Modo: {selectedIntegration.mode}
										</Badge>
										<Badge
											color={selectedIntegration.is_active ? 'green' : 'red'}
											variant='outline'>
								...
								{selectedIntegration &&
									selectedIntegration.mode !== 'read_write' && (
										<p className='mt-1 text-sm text-red-500'>
											⚠️ Solo las integraciones en modo Lectura/Escritura
											pueden sincronizar stock.
										</p>
									)}
							</div>
							...
							<div className='space-y-2'>
								<Checkbox
									...
									onChange={(event) => setSyncAllProducts(event.target.checked)}
									label='Sincronizar todos los productos de la subsidiaria'
								/>
								{syncAllProducts ? (
									<Alert variant='outline' color='emerald'>
										Se consultará el stock completo del ERP y se enviará a
										WooCommerce. Este proceso puede tardar varios minutos.
									</Alert>
								) : (
									<p className='text-sm text-gray-500'>
										Si prefieres sincronizar solo algunos SKU, ingrésalos en el
										campo inferior.
									</p>
								)}
							</div>
							...
							<div>
								...
								<p className='mt-1 text-xs text-gray-500'>
									El sistema consultará el stock actual en el ERP y lo actualizará
									en WooCommerce.
								</p>
							</div>
