		...
		<div className='relative'>
			<Dropdown>
				...
				<DropdownMenu
					...
					<div className='max-h-96 divide-y divide-dashed divide-zinc-500/30 overflow-auto px-4 py-2 dark:divide-zinc-700/40'>
						{recent.map((n) => (
							<div
								key={n.id}
					...
					<div className='flex items-center justify-between gap-2 border-t border-zinc-200 px-3 py-2 dark:border-zinc-800'>
						<div className='flex gap-1' onMouseDown={(e) => e.preventDefault()}>
							...
							<button
								type='button'
								className={`rounded-md px-2 py-1 text-xs ${tab === 'all' ? 'bg-zinc-200 text-zinc-900 dark:bg-zinc-700 dark:text-white' : 'bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300'}`}
								onClick={() => setTab('all')}
