...
export const validateFile = (file: File, opts: { maxKB?: number; allowedMimes?: string[] } = {}): FileValidationResult => {
    ...
    if (allowedMimes && allowedMimes.length > 0) {
        ...
        if (!allowedMimes.map(m => m.toLowerCase()).includes(mime)) return { ok: false, reason: 'mime' };
    }
    return { ok: true };
};
