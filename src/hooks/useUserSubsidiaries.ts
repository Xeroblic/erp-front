...
export const useUserSubsidiaries = (
    ...
    const currentUserId = useAppSelector((s) => s.auth.user?.id ?? (s.auth.user as any)?.pk ?? undefined);
        ...
        try {
            ...
            if (isSelf || !canViewUser) {
                if (authUser) {
                    ...
                    [...acc, ...vis].forEach((s: any) => { if (s?.id && !map.has(s.id)) map.set(s.id, s); });
                    ...
                    if (Array.isArray(raw) && raw.length) {
                        const normalized: UserSubsidiary[] = raw.map((s: any) => {
                            ...
                            const companyName = typeof company === 'object' ? (company?.name ?? company?.company_name ?? null) : s?.company_name ?? null;
                            return {
                                id: s.id,
                ...
                if ((!Array.isArray(rawSubs) || rawSubs.length === 0) && meData?.branch?.subsidiary) {
                    const sb = meData.branch.subsidiary;
                    if (sb?.id) rawSubs = [sb];
                }
            ...
            const resp = await ApiService.fetchData({ url: `/users/${userId}?include=access`, method: 'get' });
            ...
            [...accessSubs, ...visibleSubs].forEach((s: any) => { if (s?.id && !merged.has(s.id)) merged.set(s.id, s); });
            ...
            const message = error?.response?.data?.message || error?.message || 'Error al obtener subsidiarias del usuario';
