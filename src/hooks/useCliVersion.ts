...
const resolveVersions = async (): Promise<CliVersionPayload> => {
	const fromMemory =
		inMemoryCache && Date.now() - inMemoryCache.fetchedAt <= CLI_VERSION_CACHE_TTL_MS
			? inMemoryCache
			: null;
	...
	const shouldFetchGithub =
		!import.meta.env.DEV || import.meta.env.VITE_CLI_VERSION_CHECK_GITHUB === 'true';
	...
	inFlightPromise = (async () => {
		try {
			...
			if (shouldFetchGithub) {
				try {
					const response = await fetch(
						'https://api.github.com/repos/R4aveen/zentria-cli/releases/latest',
						{
							cache: 'no-store',
						},
					);
					...
					if (response.ok) {
						...
						const exactAsset = assets.find(
							(a: any) => String(a.name || '').toLowerCase() === expectedName,
						);
			...
			if (
				localCliVersion !== '0.0.0' &&
				githubCliVersion !== '0.0.0' &&
				compareVersions(localCliVersion, githubCliVersion) > 0
			) {
				const formattedVersion = localCliVersion.startsWith('v')
					? localCliVersion
					: `v${localCliVersion}`;
...
export const useCliVersion = () => {
	...
	const [savedVersion, setSavedVersion] = useState<string | null>(() =>
		localStorage.getItem('zentria_cli_version'),
	);
