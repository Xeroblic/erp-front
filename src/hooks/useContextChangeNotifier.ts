...
const useContextChangeNotifier = () => {
    ...
    useEffect(() => {
        ...
        if (previousContext.subsidiary && previousContext.subsidiary !== currentContext.subsidiary) {
            toast.info(`🏢 Subsidiaria cambiada: ${currentContext.subsidiary || 'Sin subsidiaria'}`, {
                position: "top-right",
                autoClose: 4000,
            });
        }
