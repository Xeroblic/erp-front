...
export function useGeoSelector(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!formik.values[fieldComuna] || String(formik.values[fieldComuna]) !== String(comunaId)) {
  ...
  useEffect(() => {
    ...
    const provinciaValida = filtered.some((p) => String(p.codigo) === String(formik.values[fieldProvincia]));
    if (!provinciaValida) {
      if (formik.values[fieldProvincia]) formik.setFieldValue(fieldProvincia, '', false);
      if (formik.values[fieldComuna]) formik.setFieldValue(fieldComuna, '', false);
    }
