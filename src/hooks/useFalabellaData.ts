	...
	useEffect(() => {
		const fetchLowStockProducts = async () => {
			try {
				...
				setError(
					err instanceof Error ? err.message : 'Error al cargar productos con stock bajo',
				);
			} finally {
				setLoading(false);
			}
	...
	useEffect(() => {
		const fetchBestSellingProducts = async () => {
			try {
				...
				setError(
					err instanceof Error ? err.message : 'Error al cargar productos más vendidos',
				);
			} finally {
				setLoading(false);
			}
	...
	useEffect(() => {
		const fetchInventorySummary = async () => {
			try {
				...
				setError(
					err instanceof Error ? err.message : 'Error al cargar resumen de inventario',
				);
			} finally {
				setLoading(false);
			}
