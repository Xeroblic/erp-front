...
export const ROLE_DESCRIPTIONS = {
	'super-admin': {
		...
		description:
			'Acceso exclusivo a la empresa principal. Puede ver y modificar datos generales. No gestiona sucursales ni sub-empresas.',
	...
	'company-admin': {
		...
		description:
			'Equivale al administrador de la empresa principal. Tiene acceso y supervisión de alto nivel.',
		scope: 'Empresa Principal',
		level: 11,
	},
	'subsidiary-admin': {
		name: 'Administrador',
		description:
			'Administra una sub-empresa o sucursal. Equivale al "Administrador del Sistema" para esa sub-empresa. Control total dentro de su ámbito.',
	...
	'warehouse-manager': {
		...
		description:
			'Acceso restringido a información relacionada con bodega. Rol operativo con permisos menores que un supervisor.',
	...
	'sales-rep': {
		name: 'Vendedor',
		description:
			'Puede visualizar productos, stock y ventas. Tiene acceso en modo solo lectura a información de bodega. No puede editar información fuera de sus competencias.',
	...
	technician: {
		name: 'Técnico',
		description:
			'Accede exclusivamente al módulo de revisiones técnicas. Puede agregar, editar o eliminar revisiones. El resto de los módulos son de solo lectura.',
	...
	'after-sales': {
		name: 'Postventa',
		description:
			'Accede en modo lectura a información relacionada con pedidos, órdenes de pedido y ventas. No puede modificar datos en otros módulos.',
	...
	cashier: {
		name: 'Cajero',
		description:
			'Puede generar y exportar cotizaciones, visualizar productos y realizar ventas.',
	...
	manager: {
		name: 'Gerente',
		description:
			'Accede a dashboards, informes y reportes. Tiene una visión general del funcionamiento del sistema.',
	...
	system: {
		name: 'Sistema',
		description:
			'Rol interno, relacionado con la configuración y lógica del sistema. No es visible para ningún usuario.',
		scope: 'Interno',
		level: 0,
	},
} as const;
...
export const hasMinimumRoleLevel = (userRoles: string[], minimumRole: string): boolean => {
	const userMaxLevel = Math.max(
		...userRoles.map(
			(role) => ROLE_DESCRIPTIONS[role as keyof typeof ROLE_DESCRIPTIONS]?.level || 0,
		),
	);
	const requiredLevel =
		ROLE_DESCRIPTIONS[minimumRole as keyof typeof ROLE_DESCRIPTIONS]?.level || 0;
